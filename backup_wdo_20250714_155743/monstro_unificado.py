...
class GerenciadorBloqueio:
    ...
    def registrar_operacao(self, acao: str, lucro: float) -> None:
        ...
        if acao not in ["BUY", "SELL"]:
            logging.debug(f"Ignorando registro de operação para ação inválida: {acao}")
            return
        ...
        if lucro < -25.0:  # Só conta como loss se for prejuízo significativo (maior que 25 reais)
            ...
            self.losses_sequencia[acao] = max(0, self.losses_sequencia[acao] - 1)  # Decrementa gradualmente
            ...
            if lucro >= MIN_LUCRO_DESBLOQUEIO and self.bloqueio_lado[acao] > 0:
                self.bloqueio_lado[acao] = max(0, self.bloqueio_lado[acao] - 1)  # Reduz bloqueio gradualmente
                logging.info(f"✅ Reduzindo bloqueio do lado {acao} por resultado não negativo")
    ...
    def obter_acao_alternativa(self, acao_original: str) -> str:
        ...
        if acao_original == "BUY":
            ...
            logging.warning(f"Ação original inválida para alternativa: {acao_original}")
            return "BUY"  # Default
...
MIN_VOLUME_BOOK = 200      # Volume mínimo no book
...
THRESHOLD_ATR_BAIXO = 25        # ATR baixo para modo lateralidade (reduzido de 50 para 25)
THRESHOLD_ENTROPIA_BAIXA = 0.2  # Entropia baixa para modo lateralidade (reduzido de 0.3 para 0.2)
THRESHOLD_ENTROPIA_ALTA = 0.8   # Entropia alta para modo explosão (aumentado de 0.7 para 0.8)
MIN_VOLUME_CRESCIMENTO = 1.2    # Mínimo crescimento de volume para modo explosão (reduzido de 1.5 para 1.2)
MAX_LOSSES_SEGUIDOS = 5         # Máximo de losses seguidos antes de modo defesa (aumentado de 3 para 5)
TEMPO_DEFESA = 15               # Minutos em modo defesa após atingir max losses (reduzido de 30 para 15)
MIN_RATIO_BOOK = 0.05           # Razão mínima entre bid/ask (reduzido de 0.1 para 0.05)
...
def calcular_entropia(volumes: List[int]) -> float:
    ...
    if not volumes:
        logging.debug("[Entropia] Lista de volumes vazia, retornando entropia 0.0")
        return 0.0
    ...
    try:
        ...
        logging.error(f"[Entropia] Erro ao converter volumes para int: {e}, volumes: {volumes[:5]}...")
        return 0.0

    if not volumes_validos:
        logging.debug("[Entropia] Não há volumes válidos (>0), retornando entropia 0.0")
        return 0.0

    resultado_entropia = entropy(volumes_validos)
    logging.debug(f"[Entropia] Entropia calculada: {resultado_entropia:.3f} (volumes: {len(volumes_validos)})")
...
def preparar_dados(df: pd.DataFrame) -> Tuple[pd.DataFrame, Optional[pd.Series]]:
    ...
    try:
        df_work = normalizar_dados(df_work, colunas_numericas, colunas_categoricas)
    except Exception as e:
        ...
        scaler = MinMaxScaler()
        df_work[colunas_numericas] = scaler.fit_transform(df_work[colunas_numericas])

    # Seleciona apenas as colunas necessárias
    todas_colunas = colunas_numericas + colunas_categoricas
    colunas_disponiveis = [col for col in todas_colunas if col in df_work.columns]
    X = df_work[colunas_disponiveis]

    # Prepara target
    y = df_work['action'].apply(lambda x: 1 if x == 'BUY' else 0) if 'action' in df_work else None
        ...
        try:
            ...
            try:
                ...
                if resultado_teste.shape != (1, 1):
                    raise ValueError(f"Formato de saída incorreto: {resultado_teste.shape}, esperado: (1, 1)")
                ...
                if np.isnan(resultado_valor) or np.isinf(resultado_valor):
                    raise ValueError("Modelo retorna valores inválidos (NaN/inf)")

                logging.info(f"✅ Modelo de IA carregado com sucesso de {caminho} ({expected_features} features)")
                logging.info(f"✅ Teste de compatibilidade passou: resultado={resultado_valor:.3f}")
                return modelo

            except Exception as feature_error:
                logging.warning(f"⚠️ Modelo incompatível ou corrompido: {feature_error}")
                logging.info(f"🔄 Recriando modelo com {expected_features} features...")

                # Backup do modelo incompatível
                timestamp = int(time.time())
                backup_path = f"{caminho}.backup_incompativel_{timestamp}"
                os.rename(caminho, backup_path)
                logging.info(f"📦 Modelo incompatível movido para: {backup_path}")
...
def verificar_e_proteger_modelo() -> bool:
    ...
    try:
        ...
        logging.info(f"🔍 Verificando integridade do modelo: {modelo_principal}")
        ...
        if os.path.exists(modelo_principal):
            ...
            logging.warning("⚠️ Modelo principal não encontrado - tentando recuperar")
            return recuperar_modelo_automaticamente()
...
def recuperar_modelo_automaticamente() -> bool:
    ...
    try:
        ...
        import glob
        backups_diarios = sorted(glob.glob(f"{modelo_principal}.backup_diario_*"), reverse=True)
        opcoes_backup.extend(backups_diarios)

        # 2. Backups com timestamp
        backups_timestamp = sorted(glob.glob(f"{modelo_principal}.backup_*"), reverse=True)
        ...
        backups_antigos = sorted(glob.glob(f"{modelo_principal}.backup_*"), reverse=True)
        ...
        for backup_path in opcoes_backup:
            try:
                ...
                logging.info(f"✅ MODELO RECUPERADO com sucesso de: {backup_path}")
...
def ler_book_csv() -> Optional[Dict[str, List[int]]]:
    ...
    try:
        if os.path.getsize(BOOK_FILE_PATH) < 4:
            logging.debug(f"[ler_book_csv] Arquivo muito pequeno: {os.path.getsize(BOOK_FILE_PATH)} bytes")
            return None
        ...
        for encoding in ['utf-16', 'utf-8', 'ascii', 'latin-1']:
            try:
                with open(BOOK_FILE_PATH, 'r', encoding=encoding) as f:
                    lines = f.readlines()
                logging.debug(f"[ler_book_csv] Sucesso com encoding {encoding}, {len(lines)} linhas")
                break
            except UnicodeDecodeError:
                continue
        else:
            logging.error(f"[ler_book_csv] Nenhuma codificação funcionou para {BOOK_FILE_PATH}")
            return None
        ...
        bids = [int(v) for v in bids_str.split(',') if v and v.strip().isdigit()] if bids_str else []
        asks = [int(v) for v in asks_str.split(',') if v and v.strip().isdigit()] if asks_str else []

        logging.debug(f"[ler_book_csv] BIDs parsed: {len(bids)} volumes, total: {sum(bids)}")
        logging.debug(f"[ler_book_csv] ASKs parsed: {len(asks)} volumes, total: {sum(asks)}")
        ...
        logging.debug(f"[ler_book_csv] Retornando dados válidos: {len(bids)} bids, {len(asks)} asks")
        return resultado
...
def inicializar_mt5() -> bool:
    ...
    if terminal_info:
        BOOK_FILE_PATH = os.path.join(terminal_info.data_path, 'MQL5', 'Files', 'book_data.csv')
...
def obter_dados_mercado(symbol: str = None, timeframe: int = TIMEFRAME) -> Tuple[Optional[float], ...]:
    ...
    try:
        ...
        if log_time:
            ...
            logging.info(f"📊 Mercado (EA): Liq:{total_volume} | Spread:{spread_atual}pts")

        # Verifica liquidez mínima
        if total_volume < MIN_VOLUME_BOOK:
            if log_time:
                logging.warning(f"❌ Liquidez insuficiente do EA: {total_volume} < {MIN_VOLUME_BOOK}")
            return (None,) * 8
        ...
        if symbol_info is None:
            ...
            if symbol_info is None:
                logging.error(f"❌ Não foi possível obter info do símbolo {symbol} mesmo após reselecionar")
            return (None,) * 8
        ...
        atr = calcular_atr(df_rates['high'].tolist(), df_rates['low'].tolist(), df_rates['close'].tolist(), 14)
        ...
        logging.debug(f"📊 EA Data - Bid Vol: {total_bid_volume}, Ask Vol: {total_ask_volume}")
...
def executar_ordem(action: str, lots: float = VOLUME_PADRAO, symbol: str = None,
    ...
    if symbol is None:
        logging.error("❌ SYMBOL não está definido! Não é possível executar ordem.")
        return None
    ...
    if symbol_info is None:
        ...
        symbol_info = get_cached_symbol_info(symbol)
        if symbol_info is None:
            logging.error(f"❌ Symbol_info ainda é None após limpar cache para {symbol}")
            return None

    logging.info(f"✅ Dados obtidos - Tick: Ask={tick.ask}, Bid={tick.bid}, Symbol: {symbol_info.name}")

    if tick is None or symbol_info is None:
        logging.warning("Dados de mercado indisponíveis após tentativas de correção")
        return None
...
def monstro_thread(mt5_ativo_param=None, modelo_ia_param=None):
    ...
    try:
        ...
        if not verificar_e_proteger_modelo():
            logging.warning("⚠️ Proteção do modelo identificou problemas - continuando com novo modelo")
        ...
        if mercado_aberto:
            ...
            if modelo_ia_local is None:
                logging.warning("⚠️ Modelo não encontrado, criando novo modelo...")
        ...
        while thread_ativo:
            try:
                ...
                if agora - ultimo_diagnostico >= 300:
                    ...
                    if not verificar_e_proteger_modelo():
                        logging.warning("⚠️ Modelo teve problemas - mas foi protegido automaticamente")
                ...
                if posicao_atual is not None:
                    ...
                    lucro_real, score_dist = obter_lucro_ultima_ordem(ticket_processado)
                    gerenciador_bloqueio.registrar_operacao(posicao_atual.tipo, lucro_real)
                    if posicao_atual.entry_context:
                        ...
                        salvar_experiencia_csv(posicao_atual.entry_context.copy(), posicao_atual.tipo, lucro_real, score_dist)
                        ...
                        logging.warning("⚠️ Contexto de entrada não encontrado em posicao_atual ao fechar.")
                    modo_operacional.registrar_resultado(lucro_real)

                    # IMPORTANTE: Reset da posição ANTES de continuar
                    posicao_atual = None
                    logging.info(f"✅ Posição {ticket_processado} processada e resetada.")
                ...
                bid_qty, ask_qty, spread, volatility, candle_type, book_data, rsi_14, volume_tick = obter_dados_mercado(SYMBOL)
                ...
                entropia_book = calcular_entropia(book_data['bids'] + book_data['asks']) if book_data else 0.0
                ...
                if (not volume_crescente(n=2, symbol=SYMBOL) and
                    ...
                    logging.info("⛔ Volume muito baixo e não crescente. Operação bloqueada.")
...
def treinar_modelo(modelo: Sequential, memoria: MemoriaExperiencias) -> None:
    ...
    try:
        ...
        df_treino = df_exp.drop(columns=["reward", "reward_norm", "score_dist"])
        ...
        if len(x_train.shape) != 2 or x_train.shape[1] != N_FEATURES:
            logging.error(f"[treinar_modelo] Shape incorreto de X_train: {x_train.shape}, esperado: (?, {N_FEATURES})")
            return

        if len(y_train.shape) != 1:
            logging.error(f"[treinar_modelo] Shape incorreto de y_train: {y_train.shape}, esperado: (?,)")
            return

        # Verifica se há valores inválidos (NaN, inf)
        if np.isnan(x_train).any() or np.isinf(x_train).any():
            logging.error("[treinar_modelo] X_train contém valores NaN ou infinitos")
            x_train = np.nan_to_num(x_train, nan=0.0, posinf=1.0, neginf=0.0)

        if np.isnan(y_train).any() or np.isinf(y_train).any():
            logging.error("[treinar_modelo] y_train contém valores NaN ou infinitos")
            y_train = np.nan_to_num(y_train, nan=0.0, posinf=1.0, neginf=0.0)

        logging.info(f"[treinar_modelo] Dados preparados com sucesso - X_train.shape = {x_train.shape}, y_train.shape = {y_train.shape}")
        logging.info(f"[treinar_modelo] Tipos finais: X_train={type(x_train)} dtype={x_train.dtype}, y_train={type(y_train)} dtype={y_train.dtype}")
        logging.debug(f"[treinar_modelo] X_train sample: {x_train[0]}")
        logging.debug(f"[treinar_modelo] y_train sample: {y_train[0]}")

    except Exception as e:
        logging.error(f"[treinar_modelo] Erro ao preparar dados para treino: {e}")
        logging.debug(f"Stack trace: {traceback.format_exc()}")
        return
    ...
    try:
        ...
        logging.info(f"[treinar_modelo] x_train: shape={x_train.shape}, dtype={x_train.dtype}, type={type(x_train)}")
        logging.info(f"[treinar_modelo] y_train: shape={y_train.shape}, dtype={y_train.dtype}, type={type(y_train)}")
        logging.info(f"[treinar_modelo] sample_weight: len={len(sample_weight)}, type={type(sample_weight)}")
        ...
        if len(x_train) > 1:
            test_x = x_train[:1]
            test_y = y_train[:1]
            logging.info(f"[treinar_modelo] Teste rápido com amostra: x_shape={test_x.shape}, y_shape={test_y.shape}")
            _ = modelo.predict(test_x, verbose=0)
            logging.info("[treinar_modelo] ✅ Teste de predição passou - iniciando fit completo")
...
def prever_acao(modelo: Sequential, X: pd.DataFrame,
    ...
    try:
        ...
        if np.isnan(x_pred).any() or np.isinf(x_pred).any():
            logging.warning("[prever_acao] Dados contêm valores NaN ou infinitos - corrigindo")
            x_pred = np.nan_to_num(x_pred, nan=0.0, posinf=1.0, neginf=0.0)

        # Verifica shape
        if len(x_pred.shape) != 2 or x_pred.shape[1] != N_FEATURES:
            logging.error(f"[prever_acao] Shape incorreto: {x_pred.shape}, esperado: (?, {N_FEATURES})")
            return "NADA", 0.0

        logging.debug(f"[prever_acao] x_pred: shape={x_pred.shape}, dtype={x_pred.dtype}")
        ...
        if memoria_experiencias:
            ...
            if razao_atual < 0.35:  # Menos de 35% de compras (mais tolerante)
                ajuste = min((0.35 - razao_atual) * 3.0, max_ajuste)  # Multiplicador mais agressivo
            ...
            # Se houver mais compras que vendas, aumenta threshold para favorecer vendas
                ...
                ajuste = min((razao_atual - 0.65) * 3.0, max_ajuste)  # Multiplicador mais agressivo
                threshold = threshold_base + ajuste + rsi_ajuste
