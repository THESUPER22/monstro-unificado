...
# Módulos locais
# from diagnostico_monstro import checar_arquivos_essenciais # Comentado para evitar erro de importação
...
@retry(stop=stop_after_attempt(MAX_RETRY_ATTEMPTS), wait=wait_exponential(multiplier=RETRY_WAIT_MULTIPLIER))
...
VOLUME_MINIMO = 50
# Atualizado para incluir 3 features de ticks
...
def calcular_volume_inteligente(volume_book_total: float) -> float:
    ...
    if volume_book_total >= 2000:
        ...
        # ========== MELHORIA 6: COLETA DE TICKS INTELIGENTE (+3% EFICÁCIA) ==========


class ColetorTicksInteligente:
    """Coleta e analisa ticks recentes para detectar microtendências."""

    def __init__(self):
        self.cache_ticks = []
    self.ultimo_update = 0
    self.cache_ttl = 2  # Cache válido por 2 segundos

    def coletar_ticks_recentes(self, symbol: str, n_ticks: int = 100) -> Optional[Dict[str, Any]]:
        """Coleta últimos N ticks com cache inteligente."""
    agora = t

    # Usa cache se ainda válido
    if agora - self.ultimo_update < self.cache_ttl and self.cache_ticks:
        return self._analisar_ticks(self.cache_ticks)

    try:
        # Coleta ticks dos últimos 60 segundos
        time_from = datetime.now() - timedelta(seconds=60)

        # Usa copy_ticks_from para coletar ticks recentes
        ticks = mt5.copy_ticks_from(
            symbol, time_from, n_ticks, mt5.COPY_TICKS_ALL)

        if ticks is None or len(ticks) == 0:
            logging.debug(f"🔍 Nenhum tick coletado para {symbol}")
            return None

        # Converte para lista de dicionários para facilitar análise
        ticks_list = []
        for tick in ticks:
            if tick['last'] > 0:  # Só ticks com preço válido
                ticks_list.append({
                    'time': tick['time'],
                    'bid': tick['bid'],
                    'ask': tick['ask'],
        ...
        if len(ticks_list) < 10:  # Mínimo de ticks para análise
            logging.debug(f"🔍 Poucos ticks válidos: {len(ticks_list)}")
            return None

        # Atualiza cache
        self.cache_ticks = ticks_list
        self.ultimo_update = agora

        return self._analisar_ticks(ticks_list)

    except Exception as e:
        logging.error(f"❌ Erro ao coletar ticks: {e}")
        return None

    def _analisar_ticks(self, ticks_list: List[Dict]) -> Dict[str, Any]:
        """Analisa ticks coletados e extrai features."""
        if not ticks_list or len(ticks_list) < 5:
            return None

        try:
            # Extrai preços e volumes
            precos = [tick['last'] for tick in ticks_list]
            volumes = [tick['volume'] for tick in ticks_list]
            spreads = [tick['ask'] - tick['bid'] for tick in ticks_list]

            # 1. DIREÇÃO DO FLUXO (-1 a +1)
            # Compara primeiro e último preço
            direcao_preco = (precos[-1] - precos[0]) / \
                             precos[0] if precos[0] > 0 else 0
            # Normaliza para WIN
            direcao_fluxo = max(-1.0, min(1.0, direcao_preco * 1000))

            # 2. INTENSIDADE DOS TICKS (0 a 1)
            # Baseada na variação de preços
            if len(precos) > 1:
                variacoes = [abs(precos[i] - precos[i-1])
                                 for i in range(1, len(precos))]
                variacao_media = sum(variacoes) / \
                                     len(variacoes) if variacoes else 0
                intensidade_ticks = min(
                    1.0, variacao_media * 100)  # Normaliza para WIN
            else:
                intensidade_ticks = 0.0

            # 3. ACELERAÇÃO DE PREÇO (0 a 1)
            # Compara velocidade primeira vs segunda metade
            meio = len(precos) // 2
            if meio > 2:
                vel_inicial = abs(
                    precos[meio] - precos[0]) / meio if meio > 0 else 0
                vel_final = abs(precos[-1] - precos[meio]) / \
                    (len(precos) - meio) if (len(precos) - meio) > 0 else 0
                aceleracao_preco = min(
                    1.0, (vel_final / vel_inicial) - 1.0) if vel_inicial > 0 else 0.0
                aceleracao_preco = max(0.0, aceleracao_preco)
            else:
                aceleracao_preco = 0.0

            # 4. SPREAD MÉDIO
            spread_medio = sum(spreads) / len(spreads) if spreads else 0

            # 5. VOLUME MÉDIO
            volume_medio = sum(volumes) / len(volumes) if volumes else 0

            resultado = {
                'qtd_ticks': len(ticks_list),
                'direcao_fluxo': round(direcao_fluxo, 4),
                'intensidade_ticks': round(intensidade_ticks, 4),
                'aceleracao_preco': round(aceleracao_preco, 4),
                'spread_medio': round(spread_medio, 5),
                'volume_medio': round(volume_medio, 2),
                'preco_inicial': precos[0],
                'preco_final': precos[-1],
                'variacao_total': round((precos[-1] - precos[0]) / precos[0] * 100, 4) if precos[0] > 0 else 0
            }

            # Log debug a cada 10 segundos
            if int(time.time()) % 10 == 0:
                logging.debug(f"🎯 Ticks: Dir={resultado['direcao_fluxo']:.3f}, "
                    f"Int={resultado['intensidade_ticks']:.3f}, "
                    f"Acel={resultado['aceleracao_preco']:.3f}")

            return resultado

        except Exception as e:
            logging.error(f"❌ Erro ao analisar ticks: {e}")
            return None


# Configurações da coleta de ticks
TICKS_ATIVO = True
TICKS_CACHE_TTL = 2  # Cache válido por 2 segundos
TICKS_QUANTIDADE = 100  # Quantidade de ticks a coletar
TICKS_JANELA_TEMPO = 60  # Janela de tempo em segundos

# Instância global do coletor de ticks
coletor_ticks = None
...
def setup_logging():
    ...
    logging.info(
    " Monstro WIN v2 iniciado! Pronto para operar Mini Índice.")
    logging.info(
    ...
    try:
        if os.path.exists(HISTORICO_CSV):
            ...
            if len(df) > 5000:  # Reduzido de 10000 para 5000
                ...
                logging.info("📊 Histórico truncado para últimas 5000 linhas")
...
def corrigir_csv_historico() -> None:
    ...
    os.rename(HISTORICO_CSV, backup_name)
    logging.warning(
    f" Arquivo muito grande ({tamanho_arquivo:.1f}MB). Movido para: {backup_name}")
    return
    ...
    'floating_profit', 'tempo_em_trade', 'direcao_fluxo', 'intensidade_ticks',
    'aceleracao_preco', 'action', 'reward']
...
def salvar_experiencia_csv(contexto: Dict[str, Any], acao: str, lucro: float, score_dist: float) -> None:
    ...
    # ========== NOVAS FEATURES DOS TICKS ==========
    # Entre -1 e 1
    'direcao_fluxo': max(-1.0, min(1.0, float(contexto.get('direcao_fluxo', 0.0)))),
    # Entre 0 e 1
    'intensidade_ticks': max(0.0, min(1.0, float(contexto.get('intensidade_ticks', 0.0)))),
    # Entre 0 e 1
    'aceleracao_preco': max(0.0, min(1.0, float(contexto.get('aceleracao_preco', 0.0)))),
    'action': acao,
    'reward': float(lucro)
    }
    ...
    logging.warning(
    " Arquivo de histórico muito grande, aguardando rotação...")
    return
...
def preparar_dados(df: pd.DataFrame) -> Tuple[pd.DataFrame, Optional[pd.Series]]:
    ...
    'direcao_fluxo', 'intensidade_ticks', 'aceleracao_preco']
...
def carregar_modelo(caminho: str = MODELO_PATH) -> Optional[Sequential]:
    ...
    return modelo
        ...
        logging.warning(
    f" Modelo incompatível ou corrompido: {feature_error}")
    logging.info(
...
def verificar_e_proteger_modelo() -> bool:
    """PROTECAO TOTAL DO MODELO - Verifica e recupera automaticamente se necessario."""
    try:
...
def recuperar_modelo_automaticamente() -> bool:
        """RECUPERACAO AUTOMATICA - Encontra e restaura backup do modelo."""
    try:
        modelo_principal = MODELO_PATH
...
import glob
    ...
    return True
        ...
        logging.warning(f"⚠️ Backup {backup_path} inválido: {e}")
        continue

    # Se chegou aqui, nenhum backup funcionou
    logging.error("❌ NENHUM BACKUP VÁLIDO ENCONTRADO!")
    logging.info("🔧 Será necessário criar um novo modelo do zero")
    return False
...
def treinar_modelo_com_dados(modelo: Sequential, df: pd.DataFrame) -> Sequential:
    """Treina o modelo com os dados disponíveis."""
    try:
        if len(df) < MIN_EXPERIENCIAS_TREINO:
            logging.warning(
                f"⚠️ Poucos dados para treino: {len(df)} < {MIN_EXPERIENCIAS_TREINO}")
            return modelo

        # Prepara dados
        X, y = preparar_dados(df)
        if X is None or y is None or len(X) == 0:
            logging.warning("⚠️ Dados preparados são inválidos")
            return modelo
        ...
        if classes_unicas < 2:
            logging.warning(
                f"⚠️ Apenas {classes_unicas} classe(s) nos dados - treino pulado")
            return modelo

        # Recompila o otimizador antes do treino (CRÍTICO para TensorFlow)
        modelo.compile(
            optimizer=Adam(learning_rate=0.001),
            loss='binary_crossentropy',
            metrics=['accuracy']
        ...
        # Treina o modelo
        logging.info(f"🎯 Iniciando treino com {len(X)} amostras")
        history = modelo.fit(
            X.values, y.values,
        ...
        logging.info(
            f"✅ Treino concluído - Loss: {loss_final:.4f}, Acc: {acc_final:.4f}")
...
def inicializar_mt5() -> bool:
    ...
    try:
        ...
        if account_info is None:
            logging.error("❌ Falha ao obter informações da conta")
            return False

        logging.info(f"✅ MT5 inicializado - Conta: {account_info.login}")
        logging.info(f"💰 Saldo: R$ {account_info.balance:.2f}")
        logging.info(f"📊 Equity: R$ {account_info.equity:.2f}")

        return True

    except Exception as e:
        logging.error(f"❌ Erro ao inicializar MT5: {e}")
        return False


def obter_dados_mercado(symbol: str) -> Optional[Dict[str, Any]]:
    """Obtém dados do mercado incluindo book de ofertas e ticks."""
    try:
        # ========== INTEGRAÇÃO MELHORIA 6: COLETA DE TICKS INTELIGENTE ==========
        dados_ticks = None
        if coletor_ticks and TICKS_ATIVO:
            dados_ticks = coletor_ticks.coletar_ticks_recentes(symbol)

        # Obtém book de ofertas
        book = retry_market_book_get(symbol)
        if book is None or len(book) == 0:
            logging.warning(f"⚠️ Book vazio para {symbol}")
            return None

        # Processa book
        bid_volumes = [item.volume for item in book if item.type == 1]
        ask_volumes = [item.volume for item in book if item.type == 2]

        if not bid_volumes or not ask_volumes:
            logging.warning("⚠️ Book sem volumes válidos")
            return None

        # Calcula métricas do book
        bid_qty = sum(bid_volumes)
        ask_qty = sum(ask_volumes)
        spread = book[0].price - book[-1].price if len(book) > 1 else 0

        # Obtém dados de preços para indicadores
        ...
        if rates is None or len(rates) < 20:
            logging.warning("⚠️ Dados de preços insuficientes")
            return None
        ...
        # Estocástico
        k_slow, d_slow = calcular_estocastico_lento(highs, lows, closes)

        # Entropia do book
        entropia = calcular_entropia(bid_volumes + ask_volumes)
        ...
        contexto = {
            ...
            'rsi_14': rsi,
            'volume_tick': volumes[-1] if volumes else 0,
            'is_in_trade': 0,  # Será atualizado depois
            'floating_profit': 0.0,  # Será atualizado depois
            'tempo_em_trade': 0,  # Será atualizado depois
            'k_slow': k_slow,
            'd_slow': d_slow
        }

        # ========== INTEGRAÇÃO FEATURES DOS TICKS ==========
        if dados_ticks:
            contexto.update({
                'direcao_fluxo': dados_ticks.get('direcao_fluxo', 0.0),
                'intensidade_ticks': dados_ticks.get('intensidade_ticks', 0.0),
                'aceleracao_preco': dados_ticks.get('aceleracao_preco', 0.0)
            })
        else:
            # Valores padrão se não conseguir coletar ticks
            contexto.update({
                'direcao_fluxo': 0.0,
                'intensidade_ticks': 0.0,
                'aceleracao_preco': 0.0
...
def calcular_atr(highs: List[float], lows: List[float], closes: List[float], period: int = 14) -> float:
...
def prever_acao(modelo: Sequential, contexto: Dict[str, Any]) -> str:
    """Prevê ação baseada no contexto usando o modelo de IA."""
    try:
        # ========== INTEGRAÇÃO MELHORIA 4: CIRCUIT BREAKER BLOQUEIA OPERAÇÕES ==========
        if circuit_breaker and CIRCUIT_BREAKER_ATIVO:
            ...
            if circuit_breaker.verificar_circuit_breakers(spread_atual):
                logging.warning(f"🚫 Circuit Breaker ativo: {circuit_breaker.motivo_bloqueio}")
                return "NADA"

        # Prepara dados para predição
        df_contexto = pd.DataFrame([contexto])
        X, _ = preparar_dados(df_contexto)

        if X is None or len(X) == 0:
            logging.warning("⚠️ Contexto inválido para predição")
            return "NADA"

        # Faz predição
        predicao = modelo.predict(X.values, verbose=0)
        probabilidade = float(predicao[0][0])

        # ========== INTEGRAÇÃO MELHORIA 2: BALANCEAMENTO AJUSTA THRESHOLD ==========
        threshold_base = 0.5
        if balanceador and BALANCEAMENTO_ATIVO:
            threshold_ajustado = balanceador.ajustar_threshold(threshold_base)
        else:
            threshold_ajustado = threshold_base

        # Ajuste adicional baseado no RSI para melhor assertividade
        rsi = contexto.get('rsi_14{e}50)
        if rsi > 70:  # Sobrecomprado - favorece SELL
            threshold_ajustado += 0.05
        elif rsi < 30:  # Sobrevendido - favorece BUY
            threshold_ajustado -= 0.05

        # Decisão final
        if probabilidade > threshold_ajustado:
            acao_decidida = "BUY"
        elif probabilidade < (1 - threshold_ajustado):
            acao_decidida = "SELL"
        else:
            acao_decidida = "NADA"

        # ========== INTEGRAÇÃO MELHORIA 3: DETECTOR DE MODO AJUSTA DECISÃO ==========
        if detector_modo and acao_decidida in ["BUY", "SELL"]:
            modo_atual = detector_modo.detectar_modo()
            if modo_atual == "CONSERVADOR":
                # Em modo conservador, só opera com alta confiança
                if abs(probabilidade - 0.5) < 0.2:  # Baixa confiança
                    acao_decidida = "NADA"
                    logging.info("🛡️ Operação cancelada por modo conservador")

        # Verifica bloqueio de lado
        if gerenciador_bloqueio.verificar_bloqueio(acao_decidida):
            acao_alternativa = gerenciador_bloqueio.obter_acao_alternativa(acao_decidida)
            logging.warning(f"🚫 Lado {acao_decidida} bloqueado, usando {acao_alternativa}")
            acao_decidida = acao_alternativa

        # Log da decisão
        logging.info(f"🎯 IA decidiu: {acao_decidida} (prob: {probabilidade:.3f}, threshold: {threshold_ajustado:.3f})")
...
def executar_ordem(symbol: str, acao: str, volume: float, sl_points: float, tp_points: float) -> Optional[int]:
    """Executa ordem no MetaTrader 5."""
    try:
        if acao not in ["BUY", "SELL"]:
            logging.warning(f"⚠️ Ação inválida para execução: {acao}")
            return None

        # ========== INTEGRAÇÃO MELHORIA 3: DETECTOR DE MODO AJUSTA PARÂMETROS ==========
        volume_ajustado = volume
        sl_ajustado = sl_points
        tp_ajustado = tp_points

        if detector_modo:
            volume_ajustado, sl_ajustado, tp_ajustado = detector_modo.ajustar_parametros_trading(
                volume, sl_points, tp_points
            )
            if detector_modo.modo_atual == "CONSERVADOR":
                logging.info(f"🛡️ Parâmetros ajustados para modo conservador: Vol={volume_ajustado}, SL={sl_ajustado}, TP={tp_ajustado}")

        # Obtém preço atual
        tick = mt5.symbol_info_tick(symbol)
        if tick is None:
            logging.error(f"❌ Não foi possível obter tick para {symbol}")
            return None

        # Define tipo de ordem e preços
        if ac") == "BUY":
            order_type = mt5.ORDER_TYPE_BUY
      price = tick.ask
            sl_price = price - (sl_ajustado * TICK_SIZE)
            tp_price = price + (tp_ajustado * TICK_SIZE)
        else:  # SELL
        ...
        # Monta request da ordem
        request = {
            "action": mt5.TRADE_ACTION_DEAL,
            "symbol": symbol,
            "volume": volume_ajustado,
            "type": order_type,
            "price": price,
            "sl": sl_price,
            "tp": tp_price,
        ...
        if result is None:
            logging.error(f"❌ Falha ao enviar ordem: {mt5.last_error()}")
            return None
        ...
        # ========== INTEGRAÇÃO MELHORIA 1: INICIA TRAILING STOP ==========
        if trailing_stop and TRAILING_ATIVO:
            trailing_stop.iniciar_trailing(result.order, acao, price, sl_price)

        # ========== INTEGRAÇÃO MELHORIA 5: INICIA MONITORAMENTO SAÍDA INTELIGENTE ==========
        if saida_inteligente and SAIDA_INTELIGENTE_ATIVA:
            saida_inteligente.iniciar_monitoramento(result.order, acao, price)

        # ========== INTEGRAÇÃO MELHORIA 2: REGISTRA OPERAÇÃO NO BALANCEADOR ==========
        if balanceador and BALANCEAMENTO_ATIVO:
            balanceador.registrar_operacao(acao)

        logging.info(f"✅ Ordem executada: {acao} {volume_ajustado} {symbol} @ {price}")
        logging.info(f"🎯 SL: {sl_price} | TP: {tp_price} | Ticket: {result.order}")
...
def monitorar_posicoes(symbol: str) -> List[Dict[str, Any]]:
    """Monitora posições abertas e aplica trailing stop e saída inteligente."""
    try:
        ...
        for posicao in posicoes:
            if posicao.magic != MAGIC_NUMBER:
                continue

            # Informações básicas da posição
            info_posicao = {
                'ticket': posicao.ticket,
                'type': 'BUY' if posicao.type == mt5.POSITION_TYPE_BUY else 'SELL',
                'volume': posicao.volume,
                'price_open': posicao.price_open,
                'price_current': posicao.price_current,
            ...
            if trailing_stop and TRAILING_ATIVO:
                novo_sl = trailing_stop.atualizar_trailing(
                    posicao.price_current, info_posicao['type']
                )
                if novo_sl and abs(novo_sl - posicao.sl) > TICK_SIZE:
                    ...
                    if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                        logging.info(f"🎯 Trailing Stop atualizado: SL {posicao.sl} → {novo_sl}")
                        info_posicao['sl'] = novo_sl

            # ========== INTEGRAÇÃO MELHORIA 5: SAÍDA INTELIGENTE ==========
            if saida_inteligente and SAIDA_INTELIGENTE_ATIVA:
                # Atualiza RSI atual para verificação
                rates = mt5.copy_rates_range(symbol, TIMEFRAME,
                                           datetime.now() - timedelta(minutes=30),
                                           datetime.now())
                if rates is not None and len(rates) > 14:
                    closes = [rate['close'] for rate in rates]
                    rsi_atual = calcular_rsi(closes)
                    saida_inteligente.atualizar_rsi(rsi_atual)

                    # Verifica se deve sair da posição
                    if saida_inteligente.verificar_saida_inteligente(
                        posicao.ticket, posicao.price_current, rsi_atual
                    ):
                        # Fecha posição por saída inteligente
                        if fechar_posicao(posicao.ticket, symbol, "SAIDA_INTELIGENTE"):
                            logging.info(f"🚪 Posição {posicao.ticket} fechada por saída inteligente")
                            saida_inteligente.finalizar_monitoramento(posicao.ticket)
                            continue

            posicoes_info.append(info_posicao)

        return posicoes_info

    except Exception as e:
        logging.error(f"❌ Erro ao monitorar posições: {e}")
        logging.debug(f"Stack trace: {traceback.format_exc()}")
        return []


def fechar_posicao(ticket: int, symbol: str, motivo: str = "MANUAL") -> bool:
    """Fecha uma posição específica."""
    try:
        # Obtém informações da posição
        posicoes = mt5.positions_get(ticket=ticket)
        if not posicoes:
            logging.warning(f"⚠️ Posição {ticket} não encontrada")
            return False

        posicao = posicoes[0]

        # Define tipo de ordem para fechamento
        ...
        if result and result.retcode == mt5.TRADE_RETCODE_DONE:
            logging.info(f"✅ Posição {ticket} fechada por {motivo} - Lucro: R$ {posicao.profit:.2f}")

            # Finaliza trailing stop se ativo
            if trailing_stop:
                trailing_stop.finalizar_trailing()

            return True
        else:
            logging.error(f"❌ Erro ao fechar posição {ticket}: {result.retcode if result else 'None'}")
            return False

    except Excep e:
        logging.error(f"❌ Erro ao fechar posição {ticket}: {e}")
        return False


def verificar_horario_operacao() -> str:
    """Verifica se está no horário de operação."""
    agora = datetime.now().time()

    # Converte strings de horário para objetos time
    pregao = dtime.fromisoformat(HORARIO_PREGAO)
    limite = dtime.fromisoformat(HORARIO_LIMITE_ORDENS)
    encerramento = dtime.fromisoformat(HORARIO_ENCERRAMENTO)
    after = dtime.fromisoformat(HORARIO_AFTER)

    if agora < pregao:
        return "PRE_ABERTURA"
    elif pregao <= agora < limite:
n "OPERACAO"
    elif limite <= agora < encerramento:
        return "LIMITE_ORDENS"
    elif encerramento <= agora < after:
"FECHAMENTO"
    else:
        return "AFTER_MARKET"


def encerrar_todas_posicoes(symbol: str, motivo: str = "ENCERRAMENTO") -> bool:
    """Encerra todas as posições do símbolo."""
    try:
        posicoes = retry_positions_get(symbol)
        if not posicoes:
            logging.info("✅ Nenhuma posição para encerrar")
            return True

        sucesso_total = True
        for posicao in posicoes:
            if posicao.magic == MAGIC_NUMBER:
                if not fechar_posicao(posicao.ticket, symbol, motivo):
                    sucesso_total = False

        return sucesso_total

    except Exception as e:
        logging.error(f"❌ Erro ao encerrar posições: {e}")
        return False

# endregion

# region [Dashboard Web]
...
def dashboard():
    ...
    <html>
    <head>
        <title>Monstro WIN v2 - Dashboard</title>
        <meta charset="utf-8">
        ...
        <script>
            ...
            function autoRefresh() {
                setInterval(refreshData, 30000); // Refresh a cada 30 segundos
            }
    ...
    <body>
        <div class="container">
            <h1>🤖 Monstro WIN v2 - Dashboard em Tempo Real</h1>
            ...
            <div class="card">
                ...
                <div id="metrics">
                    <div class="metric">
                        ...
                        <div class="metric-label">Operações Hoje</div>
                    </div>
                    <div class="metric">
                    ...
                    <div class="metric">
                        <div class="metric-value" id="current-positions">-</div>
            ...
            <div class="card">
                <h2>🎯 Status das Melhorias</h2>
                <div id="melhorias-status">
                    <div class="status online">✅ Trailing Stop Inteligente: ATIVO</div>
                    <div class="status online">✅ Balanceamento BUY/SELL: ATIVO</div>
                    <div class="status online">✅ Modos de Mercado: ATIVO</div>
                    <div class="status online">✅ Circuit Breakers: ATIVO</div>
                    <div class="status online">✅ Saída Inteligente: ATIVO</div>
                    <div class="status online">✅ Coleta de Ticks: ATIVO</div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <ul>
                    <li><a href="/api/status" target="_blank">/api/status</a> - Status completo do sistema</li>
                    <li><a href="/api/performance" target="_blank">/api/performance</a> - Métricas de performance</li>
                    <li><a href="/api/positions" target="_blank">/api/positions</a> - Posições ativas</li>
                    <li><a href="/api/melhorias" target="_blank">/api/melhorias</a> - Status das melhorias</li>
                </ul>
            </div>
        </div>
        ...
        <script>
            ...
            fetch('/api/status')
                ...
                .then(data => {
                    document.getElementById('total-operations').textContent = data.operacoes_hoje || 0;
                    document.getElementById('success-rate').textContent = (data.taxa_acerto || 0) + '%';
                    document.getElementById('daily-pnl').textContent = 'R$ ' + (data.pnl_diario || 0).toFixed(2);
                    document.getElementById('current-positions').textContent = data.posicoes_abertas || 0;
                })
...
data.positions && data.positions.length > 0) {
                        let html = '<table><tr><th>Ticket</th><th>Tipo</th><th>Volume</th><th>Preço</th><th>Atual</th><th>Lucro</th></tr>';
                        data.positions.forEach(pos => {
                            ...
                            html += `<tr>
                                ...
                                <td class="${profitClass}">R$ ${pos.profit.toFixed(2)}</td>
                            </tr>`;
                        });
                        html += '</table>';
                        document.getElementById('positions-table').innerHTML = html;
                    } else {
                        document.getElementById('positions-table').innerHTML = '<p>Nenhuma posição ativa</p>';
                    }
                })
                .catch(error => {
                    document.getElementById('positions-table').innerHTML = '<p>Erro ao carregar posições</p>';
                    console.log('Erro ao carregar posições:', error);
                });
        </script>
    </body>
    </html>
...
def api_status():
    ...
    try:
        ...
        if SYMBOL:
            posicoes = retry_positions_get(SYMBOL)
            if posicoes:
                posicoes_abertas = len([p for p in posicoes if p.magic == MAGIC_NUMBER])

        # Status das melhorias
        melhoristatus = {
            'trailing_stop': TRAILING_ATIVO,
            'balanceamento': BALANCEAMENTO_ATIVO,
            'circuit_breaker': CIRCUIT_BREAKER_ATIVO,
            'saida_inteligente': SAIDA_INTELIGENTE_ATIVA,
            'coleta_ticks': TICKS_ATIVO
        }
        ...
        return jsonify({
            ...
            'taxa_acerto': 0,  # Será calculado com base no histórico
            'melhorias': melhorias_status,
            'horario_operacao': verificar_horario_operacao()
        })
...
def api_positions():
    """API de posições ativas."""
    try:
        if not SYMBOL:
            return jsonify({'positions': []})

        posicoes_info = monitorar_posicoes(SYMBOL)

        return jsonify({
            'positions': posicoes_info,
            'count': len(posicoes_info),
            'timestamp': datetime.now().isoformat()
        })
...
def api_performance():
    """API de performance do sistema."""
    try:
        # Lê histórico se existir
        performance_data = {
            'total_operations': 0,
        ...
        if os.path.exists(HISTORICO_CSV):
            ...
            if 'reward' in df.columns and len(df) > 0:
                ...
                performance_data.update({
                    'total_operations': len(rewards),
                    'profitable_operations': len(rewards[rewards > 0]),
                    'loss_operations': len(rewards[rewards < 0]),
                    'success_rate': (len(rewards[rewards > 0]) / len(rewards) * 100) if len(rewards) > 0 else 0,
                    'total_profit': float(rewards.sum()),
                    'average_profit': float(rewards.mean()),
                    'max_profit':wards.max()),
max_loss': float(rewards.min())
                })
...
@app.route('/api/melhorias')
def api_melhorias():
    """API de status das melhorias implementadas."""
    try:
        status_melhorias = {}

        # Status do Trailing Stop
        if trailing_stop:
            status_melhorias['trailing_stop'] = {
                'ativo': TRAILING_ATIVO,
                'posicao_monitorada': trailing_stop.posicao_ativa is not None,
                'trailing_ativo': trailing_stop.trailing_ativo,
                'lucro_travado': trailing_stop.lucro_travado
            }

        # Status do Balanceador
        if balanceador:
            status_melhorias['balanceador'] = balanceador.get_status()

        # Status do Detector de Modo
        if detector_modo:
            status_melhorias['detector_modo'] = {
                'modo_atual': detector_modo.modo_atual,
                'historico_atr': len(detector_modo.historico_atr),
                'historico_entropia': len(detector_modo.historico_entropia)
            }

        # Status do Circuit Breaker
        if circuit_breaker:
            status_melhorias['circuit_breaker'] = circuit_breaker.get_status()

        # Status da Saída Inteligente
        if saida_inteligente:
            status_melhorias['saida_inteligente'] = {
                'posicoes_monitoradas': len(saida_inteligente.posicoes_monitoradas),
                'historico_rsi': len(saida_inteligente.historico_rsi)
            }

        # Status do Coletor de Ticks
        if coletor_ticks:
            status_melhorias['coletor_ticks'] = {
                'cache_valido': len(coletor_ticks.cache_ticks) > 0,
                'ultimo_update': coletor_ticks.ultimo_update,
                'cache_ttl': coletor_ticks.cache_ttl
            }

        return jsonify({
            'melhorias': status_melhorias,
            'timestamp': datetime.now().isoformat()
        })
...
def iniciar_dashboard_web():
    ...
    try:
        logging.info(f"🌐 Iniciando dashboard web na porta {PORT}")
        app.run(host='0.0.0.0', port=PORT, debug=False, use_reloader=False)
    except Exception as e:
        logging.error(f"❌ Erro no dashboard web: {e}")
...
def loop_principal():
    ...
    global SYMBOL, trailing_stop, balanceador, detector_modo, circuit_breaker, saida_inteligente, coletor_ticks

    try:
        # ========== INICIALIZAÇÃO DAS MELHORIAS ==========
  logging.info("🚀 Inicializando melhorias do Monstro WIN v2...")

        # Melhoria 1: Trailing Stop Inteligente
        trailing_stop = TrailingStopInteligente()
        logging.info("✅ Trailing Stop Inteligente inicializado")

        # Melhoria 2: Balanceador de Operaçõ
nceador = BalanceadorOperacoes()
        logging.info("✅eador BUY/SELL inicializado")

        # Melhoria 3: Detector de Modo de Mercado
        detector_modo = DetectorModoMercado()
        logging.info("✅ Detector de Modo de Mercado inicializado")

   con  # Melhoria 4: Circuit Breaker Essencial
        circuit_breaker = CircuitBreakerEssencial()
        logging.info("✅ Circuit Breaker Essencial inicializado")

  ti    # Melhoria 5: Saída Inteligente de Posições
        saida_intelinue
...
_retry_manager = RetryManager(
    max_retries=5, base_delay=0.1, max_delay=2.0)
# endregion
...
def _ler_book_csv_core() -> Optional[Dict[str, List[int]]]:
    ...
    encodings_to_try = _csv_encoding_detector.detect_encoding(
    BOOK_FILE_PATH)
    ...
    validation_result = _csv_data_validator.validate_book_data(
    dados_brutos)
...
def inicializar_mt5() -> bool:
    ...
    logging.info(" Balanceamento BUY/SELL inicializado (+2% eficácia)")

    # ========== INICIALIZAÇÃO MELHORIA 3: MODOS DE MERCADO SIMPLIFICADOS ==========
    detector_modo = DetectorModoMercado()
    logging.info(
    " Detector de Modos de Mercado inicializado (+2% eficácia)")
    ...
    circuit_breaker = CircuitBreakerEssencial()
    logging.info(
    " Circuit Breakers Essenciais inicializados (+1.5% eficácia)")

    # ========== INICIALIZAÇÃO MELHORIA 5: SAÍDA INTELIGENTE DE POSIÇÃO ==========
    saida_inteligente = SaidaInteligentePositions()
    logging.info(
    " Saída Inteligente de Posição inicializada (+1.5% eficácia)")

    # ========== INICIALIZAÇÃO MELHORIA 6: COLETA DE TICKS INTELIGENTE ==========
    coletor_ticks = ColetorTicksInteligente()
    logging.info(
    " Coletor de Ticks Inteligente inicializado (+3% eficácia)")
    logging.info(" • Coleta últimos 100 ticks em janela de 60s")
    logging.info(" • Detecta direção de fluxo, intensidade e aceleração")
    info(" • Cache inteligente com TTL de 2s")
    ...
    logging.info(" Arquivo de book WIN encontrado e acessível")
        ...
        logging.warning(f" Problema com arquivo de book WIN: {e}")
    else:
...
def obter_dados_mercado(symbol: str = None, timeframe: int = TIMEFRAME) -> Tuple[Optional[float], ...]:
    ...
    if not book_data or not book_data.get('bids') or not book_data.get('asks'):
        if log_time:
        logging.warning(" Book CSV vazio ou inválido do EA")
    return (None,) * 9
    ...
    # ========== MELHORIA 6: COLETA DE TICKS INTELIGENTE ==========
    dados_ticks = None
    if TICKS_ATIVO and coletor_ticks:
        dados_ticks = coletor_ticks.coletar_ticks_recentes(
    symbol, TICKS_QUANTIDADE)
    if dados_ticks and log_time:
        logging.info(f" Ticks: Dir={dados_ticks['direcao_fluxo']:.3f}, "
    f"Int={dados_ticks['intensidade_ticks']:.3f}, "
    f"Acel={dados_ticks['aceleracao_preco']:.3f}, "
    f"Qtd={dados_ticks['qtd_ticks']}")

    # Log detalhado dos dados do EA
    logging.debug(
    f" EA Data - Bid Vol: {total_bid_volume}, Ask Vol: {total_ask_volume}")

    return total_bid_volume, total_ask_volume, spread, atr, candle_type, book_data, rsi_14, volume_tick, dados_ticks

    except Exception as e:
        logging.error(f" Erro ao obter dados do mercado (EA): {e}")
    return (None,) * 9 # Atualizado para incluir dados_ticks
...
def verificar_book_equilibrado(bid_qty: float, ask_qty: float) -> Tuple[bool, str]:
    ...
    if max(bid_qty, ask_qty) / min(bid_qty, ask_qty) > max_ratio_pressao:
        logging.warning(f" Pressão excessiva no lado de {lado_forte}")
...
class ModoOperacional:
    ...
    self.losses_seguidos = 0
    logging.info(
    "Saindo do modo defesa apos periodo de observacao")
    else:
        return "DEFESA"
...
def registrar_resultado(self, lucro: float) -> None:
    ...
    logging.warning(
    f" {MAX_LOSSES_SEGUIDOS} losses seguidos - Entrando em modo defesa")
    else:
        self.losses_seguidos = 0
...
def executar_ordem(action: str, lots: float = VOLUME_PADRAO, symbol: str = None,
    ...
    if modo_operacional and modo_operacional.modo_atual == "DEFESA":
        logging.info("Ordem bloqueada - Modo defesa ativo")
    return None
    ...
    logging.info(
    f" Operação {action} registrada. BUY: {status['buy_count']}, SELL: {status['sell_count']} (BUY: {status['buy_percentage']:.1f}%)")

    # ========== INTEGRAÇÃO MELHORIA 5: SAÍDA INTELIGENTE DE POSIÇÃO ==========
    if saida_inteligente and SAIDA_INTELIGENTE_ATIVA:
        saida_inteligente.iniciar_monitoramento(
    resultado.order, action, preco)
    logging.info(
    f" Saída inteligente iniciada para posição {resultado.order}")

    return resultado.order
    time.sleep(0.2)

    logging.warning(
    f" Não foi possível confirmar se ordem {resultado.order} virou posição")
    return resultado.order
...
def obter_lucro_ultima_ordem(ticket_ordem_abertura: Optional[int] = None) -> Tuple[float, float]:
    ...
    if ticket_ordem_abertura is None:
        logging.warning(
    " obter_lucro_ultima_ordem chamada sem ticket_ordem_abertura. Retornando 0.0, 0.0")
    return 0.0, 0.0
    ...
    if not ordens_historico:
        logging.warning(
    f" Não foi possível obter detalhes da ordem de abertura {ticket_ordem_abertura} do histórico para calcular score_dist.")
    # Mesmo sem a ordem, retornamos o lucro encontrado.
    elif len(ordens_historico) == 0:
        logging.warning(
    f" Lista de ordens do histórico vazia para ticket {ticket_ordem_abertura} ao calcular score_dist.")
    else:
...
def atualizar_trailing_stop() -> None:
    ...
    if symbol_info is None:
        logging.warning(
    " Informações do símbolo indisponíveis para trailing")
    threading.Timer(TRAILING_INTERVALO, atualizar_trailing_stop).start()
    return
...
def atualizar_sl(ticket: int, novo_sl: float) -> bool:
    ...
    return True
        ...
        logging.warning(
    f" Falha ao mover SL. Código: {resultado.retcode} | Mensagem: {resultado.comment}")
    return False
        # endregion
...
#  logging.info("Iniciando verificacao de protecao do modelo...")
#  if not verificar_e_proteger_modelo():
    #  logging.warning(
    #  " Proteção do modelo identificou problemas - continuando com novo modelo")
...
#  if modelo_ia_local is None:
    #  logging.warning(
    #  " Modelo não encontrado, criando novo modelo...")
...
#  logging.info("Verificacao periodica do modelo...")
#  if not verificar_e_proteger_modelo():
    #  logging.warning(
    #  " Modelo teve problemas - mas foi protegido automaticamente")
...
#  else:
    #  logging.warning(
    #  " Contexto de entrada não encontrado em posicao_atual ao fechar.")
...
#  if not verificar_estado_book(SYMBOL):
    #  logging.warning(
    #  " Book em estado inválido. Tentando reiniciar...")
...
#  # Obtém dados do mercado (incluindo ticks)
#  bid_qty, ask_qty, spread, volatility, candle_type, book_data, rsi_14, volume_tick, dados_ticks = obter_dados_mercado(
#  SYMBOL)
...
#  if None in (bid_qty, ask_qty, spread, volatility, candle_type, book_data, rsi_14, volume_tick):
    #  logging.warning(
    #  " Dados do mercado incompletos. Aguardando próxima iteração...")
#  time.sleep(2)
#  continue
...
#  # ========== INTEGRAÇÃO DAS FEATURES DOS TICKS ==========
#  # Extrai features dos ticks ou usa valores padrão
#  direcao_fluxo = dados_ticks['direcao_fluxo'] if dados_ticks else 0.0
#  intensidade_ticks = dados_ticks['intensidade_ticks'] if dados_ticks else 0.0
#  aceleracao_preco = dados_ticks['aceleracao_preco'] if dados_ticks else 0.0
...
#  # Novas features dos ticks
#  "direcao_fluxo": direcao_fluxo, "intensidade_ticks": intensidade_ticks, "aceleracao_preco": aceleracao_preco
#  }
...
def prever_acao(modelo: Sequential, X: pd.DataFrame,
    ...
    if resultado_predicao is None or len(resultado_predicao) == 0:
        logging.warning(" Previsão vazia ou inválida")
    return "NADA", 0.0
    ...
    status=balanceador.get_status()
    logging.info(f" Balanceamento: BUY={status['buy_count']}, SELL={status['sell_count']}, "
...
def verificar_mercado_aberto() -> Tuple[bool, str]:
    ...
    if not symbol_info.trade_mode == mt5.SYMBOL_TRADE_MODE_FULL:
        return False, f"Símbolo não está ativo para trading ({symbol_info.trade_mode}) "
...
def verificar_spread_maximo(symbol_info: Any, tick_info: Any) -> bool:
    ...
    if spread_em_pontos > MAX_SPREAD:
        logging.warning(
    f" Spread alto: {spread_em_pontos:.1f} pontos (máx: {MAX_SPREAD})")
    return False
...
def monitorar_posicao_ativa(posicao: PosicaoAtiva) -> None:
    ...
    tick=mt5.symbol_info_tick(SYMBOL)
    if tick is None:
        logging.warning(" Tick indisponível para monitoramento")
    return
...
def fechar_posicao_score(posicao: PosicaoAtiva, motivo: str, score_atual: float) -> None:
    ...
    if resultado.retcode == mt5.TRADE_RETCODE_DONE:
        logging.info(
    f" Posição fechada por {motivo}. Score inicial: {posicao.score_inicial:.2f}, Score final: {score_atual:.2f}")
    else:
