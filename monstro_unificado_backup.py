...
class GerenciadorDeSaida:
    ...
    def verificar_condicoes_saida(self, preco_atual: float, rsi_atual: float = 50) -> Tuple[bool, str, Optional[float]]:
        ...
        lucro_em_pontos = 0.0
        # CORREÇÃO: lucro em PONTOS REAIS de preço (não ticks)
        # Antes dividia por TICK_SIZE(0.2), o que convertia para ticks
        # 80 "pontos" config = 80 pontos reais de preço = R$20 por contrato

        if self.tipo_posicao == "BUY":
            lucro_em_pontos = (
                preco_atual - self.preco_entrada)  # PONTOS REAIS
        ...
        # REGRA 1: Timeout — calibrado para TP=250pts (antes: 90s → agora: 300s)
        if tempo_em_posicao > 300 and lucro_em_pontos <= 2:
        ...
        # Com TP=250pts, pico de 75pts é oscilação normal — não deve fechar
        ...
        # REGRA 3: Estagnação — calibrado para movimentos longos (antes: 120s/10pts)
        if tempo_em_posicao > 480 and \
           0 < lucro_em_pontos < 20:
            return True, f"C12: Estagnação (lucro pequeno por {tempo_em_posicao:.0f}s)", None

        # --- Verificação das Regras de AJUSTE (Trailing Stop) ---

        # REGRA 4: Trailing Stop — gatilho e distância em PONTOS REAIS
        if lucro_em_pontos >= self.config['trailing_gatilho_pts']:
            novo_sl = 0.0
            # Distância em pontos reais (não multiplicar por tick)
            ...
            if self.tipo_posicao == "BUY":
                ...
                # Para VENDA: SL fica ACIMA do melhor preço (mais baixo atingido)
                novo_sl = self.melhor_preco + distancia_trailing_preco
...
MAX_TICKS = 2500000        # 250 pontos WIN = 2500000 ticks (alinhado ao TP amplo)
...
HEARTBEAT_LOG_INTERVALO_S = 15  # heartbeat da posição (💓) 1x a cada 15s operando
_throttle_estado = {}
...
class CooldownInteligente:
    ...
    def registrar_resultado(self, lucro: float):
        ...
        if lucro < -25.0:  # Loss significativo
            ...
            logging.warning(
                f"🔒 TRAVA PÓS-LOSS: {cooldown_segundos}s bloqueado após {self.losses_seguidos} loss(es) | "
    ...
    try:
        ...
        while thread_ativo:
            try:
                ...
                if timestamp_atual - ultimo_heartbeat >= 300:  # 5min (o pulso de 60s já mostra vida)
                ...
                if sniper_total < SNIPER_VOLUME_MIN or sniper_ratio < SNIPER_RATIO_MIN:
                    if _log_periodico('standby', 300):  # 1x a cada 5min (pulso já mostra vida)
                        logging.info(
                ...
                if acao_para_executar in ["BUY", "SELL"]:
                    ...
                    if lado_dominante and acao_para_executar != lado_dominante:
                        ...
                        time.sleep(5)  # re-checa a cada 5s (não precisa 1s p/ não brigar)
                        continue
...
if __name__ == "__main__":
    ...
    logging.info("⚙️ Horário: 09:00-12:00 e 15:00-17:30 | Treino só com lucro | Aprendizado PRESERVADO entre reinícios")
