...
TRAILING_ATIVO = True
TRAILING_GATILHO = 30      # pontos WIN — só ativa após lucro real (era 20)
TRAILING_DISTANCIA = 15    # pontos WIN — respira sem violinar (era 10)
TRAILING_PERCENTUAL_TRAVA = 0.7  # Trava 70% do lucro quando > 30 pontos
...
def executar_ordem(action: str, lots: float = VOLUME_PADRAO, symbol: str = None,
    ...
    if contexto_atual:
        ...
        if volume_total >= 5000:  # VOLUME MONUMENTAL - 10cc
            sl_points_dinamico = 30   # SL agressivo — mercado com muita liquidez
            tp_points_dinamico = 70   # TP 1:2.3 — aproveita o movimento forte
            logging.info(
                f"🚀 VOLUME MONUMENTAL (5000cc+): SL={sl_points_dinamico}, TP={tp_points_dinamico} (R/R 1:2.3)")
        elif volume_total >= 3000:  # LIQUIDEZ TOP - 8cc
            sl_points_dinamico = 30   # SL controlado
            tp_points_dinamico = 65   # TP 1:2.2
            logging.info(
                f"🏆 LIQUIDEZ TOP (3000cc+): SL={sl_points_dinamico}, TP={tp_points_dinamico} (R/R 1:2.2)")
        elif volume_total >= 2000:  # LIQUIDEZ MÉDIA-ALTA - 6cc
            sl_points_dinamico = 30   # SL controlado
            tp_points_dinamico = 60   # TP 1:2.0
            logging.info(
                f"⭐ LIQUIDEZ MÉDIA-ALTA (2000cc+): SL={sl_points_dinamico}, TP={tp_points_dinamico} (R/R 1:2.0)")
        else:  # LIQUIDEZ BAIXA/MÉDIA - 2-5cc
            sl_points_dinamico = 30   # SL padrão — precisa de 34% de acerto para lucrar
            tp_points_dinamico = 60   # TP 1:2.0 — matematicamente viável com 35%+ de acerto
            logging.info(
...
def monstro_thread(mt5_ativo_param=None, modelo_ia_param=None):
    ...
    try:
        ...
        config_saida = {
            ...
            'trailing_gatilho_pts': 30,          # Trailing só ativa após 30pts de lucro
            'trailing_distancia_pts': 15         # 15pts de distância — respira sem violinar
        }
            ...
            try:
                ...
                if timestamp_atual - ultimo_heartbeat >= 30:
                    if horario_permitido():
                        # Dentro do horário: loga normalmente
                        status_bloqueio = gerenciador_bloqueio.get_status()
                        logging.info(
                ...
                if monstro_position_active:
                    ...
                    if tick and posicao_atual is not None:
                        ...
                        if deve_sair:
                            ...
                            # Tenta fechar com até 3 tentativas em caso de falha de conexão
                            fechou = False
                ...
                if primeira_operacao:
                    ...
                    try:
                        ...
                        if sistema_confluencia:
                            ...
                            # HIERARQUIA: Se IA retornou NADA por VETO MATEMÁTICO, confluência NÃO sobrescreve
                            veto_ativo = getattr(
                                prever_acao, '_ultimo_veto', False)

                            if confluencia_info['acao'] != "NADA" and not veto_ativo:
                                if confluencia_info['acao'] != acao_predita:
                                    ...
                                    # Confluência confirma IA - usa a maior confiança disponível
                                    acao_para_executar = acao_predita
                                    # Se confianca_predita for 0.0, usa a da confluência como base
                                ...
                                # VETO MATEMÁTICO ativo - confluência não pode sobrescrever
...
def fechar_posicao_atual(motivo: str = "Fechamento manual") -> bool:
    """Fecha a posição atual ativa com reconexão automática e fallback de filling."""
    ...
    try:
        ...
        # Tenta com IOC primeiro, depois RETURN como fallback
        for filling in [mt5.ORDER_FILLING_IOC, mt5.ORDER_FILLING_RETURN]:
            request = {
                "action": mt5.TRADE_ACTION_DEAL,
            ...
            if resultado is None:
                logging.warning(
                    f"⚠️ order_send retornou None (filling={filling}), tentando reconectar...")
                reconectar_mt5()
                time.sleep(0.5)
                # Atualiza tick após reconexão
                tick = mt5.symbol_info_tick(SYMBOL)
                if tick:
                    preco = tick.bid if posicao_atual.tipo == "BUY" else tick.ask
                    request["price"] = preco
            ...
            if resultado is not None and resultado.retcode == mt5.TRADE_RETCODE_DONE:
                logging.info(
                    f"✅ Posição {posicao_atual.ticket} fechada: {motivo}")
                return True
            elif resultado is not None:
                logging.warning(
                    f"⚠️ Retcode {resultado.retcode} com filling={filling}: {resultado.comment}")

        logging.error(
            f"❌ Falha ao fechar posição após todos os métodos: {motivo}")
        return False
...
if __name__ == "__main__":
    ...
    logging.info("📋 ✅ PA1: Trava de Horário: 09:00-12:30 e 15:00-17:30")
    logging.info(
        "📋 ✅ PA2: Filtro de Memória: Só treina com experiências lucrativas")
    logging.info("📋 ✅ PA3: Reset de IA: Limpando memória para começar do zero")

    # ✅ PA3: Reset de memória foi executado na primeira inicialização.
    # DESATIVADO após primeiro dia de dados limpos — preserva aprendizado acumulado.
