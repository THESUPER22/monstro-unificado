...
class SistemaConfluencia:
    ...
    def verificar_confluencia(self, contexto: Dict[str, Any], probabilidade_ia: float, acao_ia: str) -> Dict[str, Any]:
        ...
        if bid_qty > 0 and ask_qty > 0:
            ...
            if ratio_book > 1.3:  # Muito mais compradores
            ...
            elif ratio_book < 0.77:  # Muito mais vendedores
                sinais_sell.append("BOOK_DESEQUILIBRIO")
                score_confluencia += 25
            elif ratio_book < 0.87:  # Moderadamente mais vendedor
        ...
        # Determina ação baseada na confluência
        if total_sinais_buy >= 3 and total_sinais_buy > total_sinais_sell:
            acao_confluencia = "BUY"
            confianca_confluencia = min(score_confluencia / 100.0, 1.0)
        elif total_sinais_sell >= 3 and total_sinais_sell > total_sinais_buy:
...
# Força recriação do scaler para compatibilidade com 10 features
...
def resetar_scaler_global():
    ...
    scaler_global = None
    logging.info(
        "🔄 Scaler global resetado para compatibilidade com 10 features")


def forcar_recreacao_scaler():
    """Força recriação imediata do scaler com dados dummy de 9 features numéricas."""
    global scaler_global
    import numpy as np
    from sklearn.preprocessing import MinMaxScaler

    # Cria dados dummy com 9 features numéricas para treinar o scaler
    dados_dummy = np.random.random((5, 9))
    scaler_global = MinMaxScaler()
    scaler_global.fit(dados_dummy)
    logging.info(
        "🔧 Scaler global recriado com 9 features numéricas usando dados dummy")
...
def obter_dados_mercado(symbol: str = None, timeframe: int = TIMEFRAME) -> Tuple[Optional[float], ...]:
    ...
    try:
        ...
        # Obtém preço de fechamento da última vela para detector de tendência
        close_price = last_candle[4]  # close price da última vela
...
def executar_ordem(action: str, lots: float = VOLUME_PADRAO, symbol: str = None,
    ...
    if contexto_atual:
        ...
        # Define SL/TP baseado na qualidade
        if score_qualidade >= 8:  # Setup ULTRA PREMIUM
            sl_points_dinamico = 40  # SL menor (mais agressivo)
            tp_points_dinamico = 120  # TP maior (busca mais lucro)
            logging.info(
                f"🏆 SETUP ULTRA PREMIUM: SL={sl_points_dinamico}, TP={tp_points_dinamico}")
        elif score_qualidade >= 6:  # Setup PREMIUM
            sl_points_dinamico = 50  # SL moderado
            tp_points_dinamico = 80   # TP alto
            logging.info(
                f"⭐ SETUP PREMIUM: SL={sl_points_dinamico}, TP={tp_points_dinamico}")
        else:  # Setup BOM
            sl_points_dinamico = 60  # SL conservador
            tp_points_dinamico = 50   # TP moderado
            logging.info(
                f"✅ SETUP BOM: SL={sl_points_dinamico}, TP={tp_points_dinamico}")
    else:
...
LIMITE_EXPERIENCIAS_PARA_TREINO = 3  # Treina a cada 3 experiências novas
...
def treinar_modelo(modelo: Sequential, memoria: MemoriaExperiencias) -> Sequential:
    ...
    try:
        ...
        X_train, y_train_series = preparar_dados(df_treino, treino=True)
...
def filtros_alta_acertividade(contexto_completo: Dict) -> Tuple[bool, str]:
    ...
    # FILTRO 2: Entropia ALTA (desequilíbrio forte) - MAIS RESTRITIVO
    entropia = contexto_completo.get('entropia_book', 0)
    if entropia < 0.5:  # Só opera com desequilíbrio FORTE
        return False, f"Book equilibrado demais: entropia {entropia:.3f} < 0.5"

    # FILTRO 3: ATR ALTO (volatilidade real) - MAIS RESTRITIVO
    atr = contexto_completo.get('volatility', 0)  # ATR está como 'volatility'
    if atr < 45:  # Só opera em mercado VOLÁTIL
        return False, f"Mercado lateral demais: ATR {atr:.1f} < 45"

    # FILTRO 4: RSI confirmando direção
    ...
    # SÓ OPERA COM SCORE ALTO (mínimo 7 de 11 possíveis) - SEGUIR BIG PLAYERS
    if score_qualidade < 7:
        return False, f"Setup de baixa qualidade: score {score_qualidade}/11 < 7"

    logging.info(
        f"✅ SETUP PREMIUM APROVADO! Score: {score_qualidade}/11 | Vol: {volume_total} | Entropia: {entropia:.3f} | ATR: {atr:.1f} | RSI: {rsi:.1f}")
    return True, f"Setup premium aprovado (score {score_qualidade}/11)"
...
def monitorar_posicao_ativa(posicao: PosicaoAtiva) -> None:
    ...
    if tempo_posicao > 120 and lucro_atual <= 15:  # 2 min sem evoluir
        logging.info(
...
if __name__ == "__main__":
    ...
    # Reseta scaler global para compatibilidade com 9 features numéricas
    resetar_scaler_global()
    # Força recriação imediata do scaler
    forcar_recreacao_scaler()

    # Força recriação imediata do scaler com 10 features
    forcar_recreacao_scaler()
