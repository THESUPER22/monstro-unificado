...
class GerenciadorDeSaida:
    ...
    def verificar_condicoes_saida(self, preco_atual: float, rsi_atual: float = 50) -> Tuple[bool, str, Optional[float]]:
        ...
        # REGRA 1: Timeout sem evolução (C12 - Mais agressivo)
        # Reduzido de 120s para 90s e lucro mínimo de 5pts para 2pts
        if tempo_em_posicao > 90 and lucro_em_pontos <= 2:
            return True, f"C12: Timeout ({tempo_em_posicao:.0f}s) sem evolução (lucro <= 2pts)", None

        # REGRA 2: Proteção de lucro (perda de % do pico) (C12 - Mais agressivo)
        # Começa a proteger mais cedo (15pts) e sai se perder 25% do pico (era 30%)
        if self.lucro_maximo_pontos > 15 and \
           lucro_em_pontos < self.lucro_maximo_pontos * 0.75:
            return True, f"C12: Proteção de Lucro (caiu de {self.lucro_maximo_pontos:.1f}pts - 25% do pico)", None

        # REGRA 3: Estagnação (lucro pequeno por muito tempo) (C12 - Mais agressivo)
        # Reduzido de 180s para 120s e lucro pequeno de 15pts para 10pts
        if tempo_em_posicao > 120 and \
           0 < lucro_em_pontos < 10:
            return True, f"C12: Estagnação (lucro pequeno por {tempo_em_posicao:.0f}s)", None
    ...
    def calcular_expectativa_contexto(self, contexto_atual: dict, acao_proposta: str) -> dict:
        ...
        for exp in experiencias:
            ...
            if candle_atual != candle_exp:
                # Permite alguns tipos similares
            ...
            if abs(ratio_atual - ratio_exp) > 0.5:  # Diferença significativa na pressão
                similar = False
...
# ========== TRAVA DE HORÁRIO - IMPLEMENTAÇÃO DO PLANO DE AÇÃO ==========

def horario_permitido() -> bool:
    """
    TRAVA DE HORÁRIO: Só permite operações entre 09:00-10:00 e 15:00-17:30
    Evita o 'moedor' das 10h e do almoço conforme sugestão do plano de ação.
    """
...
class GerenciadorDeSaida:
    ...
    def verificar_condicoes_saida(self, preco_atual: float, rsi_atual: float) -> Tuple[bool, str, Optional[float]]:
        ...
        if lucro_em_pontos >= 15:
            novo_sl = 0.0
            # Distância reduzida para 5 pontos (era 10)
            distancia_trailing_preco = 5 * TICK_SIZE
...
# ========== RESET DE MEMÓRIA DA IA - IMPLEMENTAÇÃO DO PLANO DE AÇÃO ==========

def resetar_memoria_ia():
    """
    RESET DE IA: Limpa memória de experiências para começar aprendizado do zero
...
def _ler_book_csv_core() -> Optional[Dict[str, List[Dict[str, float]]]]:
    ...
    # CORREÇÃO C7 (Parte 2): Limpeza do cache do sistema operacional (apenas para Linux/macOS)
    # Tenta forçar a limpeza do cache de leitura do arquivo, se o problema for a persistência de dados antigos.
    try:
        if sys.platform != 'win32':
            os.sync()  # Força a escrita de buffers
            # Comando para limpar o cache de página específico do arquivo (Linux)
            # Isso é complexo e perigoso, vamos nos ater ao 'buffering=1' e garantir que o arquivo seja fechado.
            pass
    except Exception as e:
        logging.debug(f"Falha na tentativa de limpar cache de leitura: {e}")
    ...
    for encoding in ['utf-16-le', 'utf-8', 'utf-16', 'latin1']:
        try:
            # CORREÇÃO C7: Adiciona buffering=1 para forçar a leitura do disco, contornando possível cache do SO/Python.
            with open(BOOK_FILE_PATH, 'r', encoding=encoding, buffering=1) as f:
...
def filtros_alta_acertividade(contexto_completo: Dict) -> Tuple[bool, str]:
    ...
    # CORREÇÃO C8: FILTRO DE QUALIDADE TEMPORARIAMENTE DESATIVADO PARA GERAR MAIS EXPERIÊNCIAS
    # SDO PARA APRENDIZADO (mínimo 6 de 11 possíveis) - SELETIVIDADE BALANCEADA
    # CORREÇÃO CRÍTICA (C10): Filtro de qualidade REATIVADO com limite mais baixo (>= 4) para reduzir trades ruins
    # Reativa o filtro, mas com limite baixo para permitir trades de média qualidade
    if score_qualidade < 4:
        CONTADOR_OPERACOES_REJEITADAS += 1

        # APRENDIZADO FORÇADO: Mantém a lógica de forçar após rejeições
        ...
        logging.info(
            f"❌ C10: Filtro de qualidade REATIVADO. Score {score_qualidade}/11 < 4. Operação bloqueada.")
...
def prever_acao(modelo: Sequential, X: pd.DataFrame,
    ...
    try:
        # ========== TRAVA DE HORÁRIO - PRIORIDADE MÁXIMA ==========
        if not horario_permitido():
            horario_atual = datetime.now().strftime("%H:%M")
            logging.info(
                f"🚫 HORÁRIO BLOQUEADO: {horario_atual} - Só opera 09:00-10:00 e 15:00-17:30")
            return "NADA", 0.0
        ...
        if contexto_completo:
            # Testa ambas as ações para ver qual tem melhor expectativa
            ...
            if expectativa_buy['expectativa'] <= 0 and expectativa_sell['expectativa'] <= 0:
                logging.warning(
                    f"🚫 VETO MATEMÁTICO: Expectativas negativas - BUY: {expectativa_buy['expectativa']:.2f}, SELL: {expectativa_sell['expectativa']:.2f}")
                return "NADA", 0.0

            # Se apenas uma tem expectativa positiva, força essa ação
            if expectativa_buy['expectativa'] > 0 and expectativa_sell['expectativa'] <= 0:
                # Mínimo de dados para confiança
                if expectativa_buy['trades_similares'] >= 3:
                    logging.info(
                        f"🎯 FORÇA BUY por expectativa: {expectativa_buy['expectativa']:.2f} (similares: {expectativa_buy['trades_similares']})")
            ...
            if expectativa_sell['expectativa'] > 0 and expectativa_buy['expectativa'] <= 0:
                ...
                if expectativa_sell['trades_similares'] >= 3:
                    logging.info(
                        f"🎯 FORÇA SELL por expectativa: {expectativa_sell['expectativa']:.2f} (similares: {expectativa_sell['trades_similares']})")
                    # Converte para probabilidade
...
def salvar_experiencias_json(experiencias: List[Tuple[Dict[str, Any], str, float, float]], arquivo: str = "experiencias.json") -> None:
    ...
    FILTRO DE MEMÓRIA: Só salva experiências com lucro > 0 conforme plano de ação.
    """
    try:
        dados = []
        ...
        for contexto, acao, lucro, score_dist in experiencias:
            # FILTRO DE MEMÓRIA: Só salva se lucro > 0
            if lucro > 0:
                dados.append({
                    "contexto": contexto,
                    "acao": acao,
        ...
        logging.info(
            f"✅ FILTRO DE MEMÓRIA: {experiencias_positivas}/{experiencias_totais} experiências positivas salvas em {arquivo}")
...
if __name__ == "__main__":
    ...
    logging.info("🎯 IMPLEMENTANDO PLANO DE AÇÃO PARA RESOLVER PERDAS DIÁRIAS")
    logging.info("📋 1. Trava de Horário: 09:00-10:00 e 15:00-17:30")
    logging.info(
        "📋 2. Filtro de Memória: Só treina com experiências lucrativas")
    logging.info("📋 3. Reset de IA: Limpando memória para começar do zero")

    # Reset da memória da IA conforme plano de ação
    resetar_memoria_ia()
