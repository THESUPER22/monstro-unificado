...
def rodar_backtest_win_expandido():
    ...
    print(f"Puxando 250 dias uteis de historico M1 para {symbol}...")
    ...
    media_pts_t = res_df['pts_tempo'].mean()

    wins_a = res_df['win_alvo'].sum()
    win_rate_a = (wins_a / total_trades) * 100
    saldo_r_a = res_df['lucro_r_alvo'].sum()
    media_pts_a = res_df['pts_alvo'].mean()

    print("\n==================================================")
    print(f"BACKTEST MODELO C (WIN 250 DIAS) - CORPO >= 80 PTS")
    print("==================================================")
    print(f" Ativo Analisado:         {symbol}")
    print(f" Total de Trades:         {total_trades} (Amostra Estatistica Segura)")
    print("--------------------------------------------------")
    print(" SAIDA NO TEMPO (09:15):")
    print(f"   Win Rate:              {win_rate_t:.1f}% ({wins_t}W / {total_trades - wins_t}L)")
    print(f"   Saldo Liquido (R$):    R$ {saldo_r_t:.2f} (10 minis / R$1,20 custo)")
    print(f"   Media/Trade (pts):     +{media_pts_t:.1f} pts")
    print("--------------------------------------------------")
    print(" SAIDA POR ALVO FIXO (1:1):")
    print(f"   Win Rate:              {win_rate_a:.1f}% ({wins_a}W / {total_trades - wins_a}L)")
    print(f"   Saldo Liquido (R$):    R$ {saldo_r_a:.2f} (10 minis / R$1,20 custo)")
    print(f"   Media/Trade (pts):     +{media_pts_a:.1f} pts")
    print("==================================================\n")

    print("Ultimas 10 Execucoes:")
    print(res_df[['data', 'tipo', 'corpo_0900', 'sl_pts', 'pts_tempo', 'lucro_r_tempo', 'pts_alvo', 'lucro_r_alvo']].tail(10).to_string(index=False))

if __name__ == "__main__":
    rodar_backtest_win_expandido()
