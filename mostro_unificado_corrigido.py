...
# Volume padrão (5 contratos WIN)
        ...
        HORARIO_ENCERRAMENTO = config.get(
            "horarios", {}).get("encerramento", "18:20")
...
#
 # Usa copy_ticks_from para coletar ticks recentes
    ...
    logging.debug(f" Nregio tick coletado para {symbol}")
    return None

 # Converte para lista de dictn [Loggingilitar análise
    ticks_list=[]
    for tick in ticks:
...
# region [Modelo Neural]
 ...
 # Verifica se modelo principal existe
    ...
    # Testa se o modelo pode ser carregado
        ...
        logging.warning(f" Modelo principal corrompido: {e}")
 # Modelo existe mas está corrompido - tenta recuperar
        return recuperar_modelo_automaticamente()
        else:
        logging.warning(
        " Modelo principal não encontrado - tentando recuperar")
        return recuperar_modelo_automaticamente()
