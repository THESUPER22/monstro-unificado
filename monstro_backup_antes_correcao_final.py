...
# 🔧 CORREÇÃO TIMESTAMP: Corrige timestamp sem aspas do EA
_TS_FIX_RE = re.compile(
    r'("timestamp")\s*:\s*([0-9]{4}\.[0-9]{2}\.[0-9]{2}\s+[0-9]{2}:[0-9]{2}(?::[0-9]{2})?)'
)


def _fix_timestamp(json_text: str) -> str:
    """Corrige timestamp sem aspas: 2025.08.27 13:02 -> "2025.08.27 13:02" """
    return _TS_FIX_RE.sub(r'\1:"\2"', json_text)
...
class GerenciadorDeSaida:
    """
    🛡️ UNIFICA E GERENCIA TODAS AS LÓGICAS DE SAÍDA DE UMA POSIÇÃO:
    ...
    def verificar_condicoes_saida(self, preco_atual: float, rsi_atual: float = 50) -> Tuple[bool, str, Optional[float]]:
        """
        ...
        if self.tipo_posicao == "BUY":
            lucro_em_pontos = (preco_atual - self.preco_entrada) / TICK_SIZE
            if preco_atual > self.melhor_preco:
                self.melhor_preco = preco_atual
        else:  # SELL
            lucro_em_pontos = (self.preco_entrada - preco_atual) / TICK_SIZE
            if preco_atual < self.melhor_preco:
                self.melhor_preco = preco_atual
        ...
        # REGRA 1: Timeout sem evolução
        if tempo_em_posicao > self.config['timeout_sem_evolucao_s'] and lucro_em_pontos <= self.config['lucro_minimo_evolucao_pts']:
            return True, f"Timeout ({tempo_em_posicao:.0f}s) sem evolução", None

        # REGRA 2: Proteção de lucro (perda de % do pico)
        if self.lucro_maximo_pontos > self.config['pico_minimo_protecao_pts'] and \
           lucro_em_pontos < self.lucro_maximo_pontos * (1 - self.config['percentual_perda_pico']):
            return True, f"Proteção de Lucro (caiu de {self.lucro_maximo_pontos:.1f}pts)", None

        # REGRA 3: Estagnação (lucro pequeno por muito tempo)
        if tempo_em_posicao > self.config['tempo_max_estagnacao_s'] and \
           0 < lucro_em_pontos < self.config['lucro_max_estagnacao_pts']:
            return True, f"Estagnação (lucro pequeno por {tempo_em_posicao:.0f}s)", None

        # --- Verificação das Regras de AJUSTE (Trailing Stop) ---

        # REGRA 4: Trailing Stop
        if lucro_em_pontos >= self.config['trailing_gatilho_pts']:
            novo_sl = 0.0
            distancia_trailing_preco = self.config['trailing_distancia_pts'] * TICK_SIZE
            if self.tipo_posicao == "BUY":
                # Para COMPRA: SL fica ABAIXO do melhor preço (mais alto atingido)
                novo_sl = self.melhor_preco - distancia_trailing_preco
            else:  # SELL
                # 🔧 CORREÇÃO CRÍTICA: Para VENDA, SL fica ACIMA do melhor preço (mais baixo atingido)
                # Melhor preço é o MENOR atingido, SL deve ficar ACIMA para proteger
                novo_sl = self.melhor_preco + distancia_trailing_preco

            # Validação adicional para garantir SL válido
            posicao_mt5 = mt5.positions_get(ticket=self.posicao_monitorada)
            if posicao_mt5 and len(posicao_mt5) > 0:
                sl_atual = posicao_mt5[0].sl
                # Para SELL, novo SL deve ser MENOR que atual para ser melhoria
                if self.tipo_posicao == "SELL" and novo_sl < sl_atual:
                    logging.info(
                        f"🔧 Trailing SELL: Novo SL {novo_sl:.2f} (melhoria de {sl_atual:.2f})")
                    return False, "Ajuste de Trailing Stop", novo_sl
                # Para BUY, novo SL deve ser MAIOR que atual para ser melhoria
                elif self.tipo_posicao == "BUY" and novo_sl > sl_atual:
                    logging.info(
                        f"🔧 Trailing BUY: Novo SL {novo_sl:.2f} (melhoria de {sl_atual:.2f})")
                    return False, "Ajuste de Trailing Stop", novo_sl
...
# 🔧 CIRCUIT BREAKERS CORRIGIDOS - ADAPTADO PARA WIN
MAX_LOSS_DIARIO = config.get("risk_management", {}).get(
    # 🔧 REDUZIDO: Limite de perda diária em reais (era -1000)
...
# 🔧 CONFIGURAÇÕES DE APRENDIZADO OTIMIZADAS (CORREÇÃO CRÍTICA)
# 🔧 AUMENTADO: Mínimo de experiências para começar treino (era 3)
MIN_EXPERIENCIAS_TREINO = 50
MAX_EXPERIENCIAS_MEMORIA = 1000  # Máximo de experiências na memória
# 🔧 AUMENTADO: Número de épocas por treino (era 3)
EPOCHS_TREINO = 5
# 🔧 AUMENTADO: Tamanho do batch de treino (era 32)
BATCH_SIZE = 64
...
# 🔧 AUMENTADO: Peso das experiências no replay (era 0.3)
PESO_REPLAY = 1.0
...
# 🔧 NOVA MELHORIA: SISTEMA ANTI-OVERTRADING


class AntiOvertradingFilter:
    """Sistema para prevenir overtrading e operações excessivas."""
    ...
    def can_trade(self, proposed_action: str) -> Tuple[bool, str]:
        ...
        if self.blocked_until and now < self.blocked_until:
            ...
            return False, f"Cooldown ativo por {remaining:.0f}s"
        ...
        if self.last_trade_time:
            ...
            if time_since_last < self.min_interval_seconds:
                return False, f"Intervalo mínimo: {self.min_interval_seconds - time_since_last:.0f}s restantes"

        # Verifica limite por hora
        hour_ago = now - timedelta(hours=1)
        self.trades_this_hour = [
            t for t in self.trades_this_hour if t > hour_ago]
        if len(self.trades_this_hour) >= self.max_trades_per_hour:
            return False, f"Limite de {self.max_trades_per_hour} trades/hora atingido"

        # Detecta reversões rápidas
        if self.detect_rapid_reversals(proposed_action):
            self.blocked_until = now + \
                timedelta(minutes=self.reversal_cooldown_minutes)
            return False, f"Reversão rápida detectada - cooldown {self.reversal_cooldown_minutes}min"
    ...
    def detect_rapid_reversals(self, proposed_action: str) -> bool:
        """Detecta reversões rápidas (BUY→SELL→BUY)."""
        if len(self.last_actions) < 2:
            return False

        # Verifica padrão BUY→SELL→BUY ou SELL→BUY→SELL
        actions = list(self.last_actions) + [proposed_action]
        if len(actions) >= 3:
            if (actions[-3] == actions[-1] and
                actions[-3] != actions[-2] and
                actions[-2] in ['BUY', 'SELL']):
                return True
...
class CircuitBreakerEssencial:
    ...
    def registrar_resultado(self, lucro: float):
        ...
        # 🔧 CORREÇÃO: Atualiza losses seguidos e rastreia último lucro
        self.ultimo_lucro = lucro  # Para CB4 (loss individual)
    ...
    def verificar_circuit_breakers(self, spread_atual: float) -> bool:
        """🔧 CORREÇÃO CRÍTICA: Verifica se algum circuit breaker foi ativado."""

        # 🔧 CB1: 3 losses seguidos - REATIVADO (era desabilitado)
        if self.losses_seguidos >= 3:
            self.bloqueado = True
            self.motivo_bloqueio = f"🚨 3 losses seguidos (atual: {self.losses_seguidos})"
            return True

        # 🔧 CB2: Loss diário excessivo - LIMITE REDUZIDO para R$500 (era R$1000)
        if self.loss_diario_atual <= -500.0:
            self.bloqueado = True
            self.motivo_bloqueio = f"🚨 Loss diário: R${self.loss_diario_atual:.2f}"
            return True

        # 🔧 CB3: Spread muito alto - LIMITE REDUZIDO para 10 pontos (era 20)
        if spread_atual > 10:
            self.bloqueado = True
            self.motivo_bloqueio = f"🚨 Spread alto: {spread_atual:.1f} pontos"
            return True

        # 🔧 CB4: Loss individual excessivo - NOVO CIRCUIT BREAKER
        if hasattr(self, 'ultimo_lucro') and self.ultimo_lucro <= -100.0:
            self.bloqueado = True
            self.motivo_bloqueio = f"🚨 Loss individual: R${self.ultimo_lucro:.2f}"
            return True
    ...
    def get_status(self) -> dict:
        ...
        # 🔧 CONFIGURAÇÕES DOS CIRCUIT BREAKERS CORRIGIDAS
        CIRCUIT_BREAKER_ATIVO = True
        # 🔧 REATIVADO: Stop após 3 losses seguidos
        MAX_LOSSES_SEGUIDOS_CB = 3
        # 🔧 REDUZIDO: Stop se spread > 10 pontos WIN (era 20)
        SPREAD_MAXIMO_CB = 10
        # 🔧 REDUZIDO: Stop se perda diária > R$500 (era R$1000)
        LOSS_DIARIO_CB = -500.0
        LOSS_INDIVIDUAL_CB = -100.0  # 🔧 NOVO: Stop se loss individual > R$100
...
class SistemaConfluencia:
    ...
    def verificar_confluencia(self, contexto: Dict[str, Any], probabilidade_ia: float, acao_ia: str) -> Dict[str, Any]:
        ...
        # ========== DECISÃO FINAL DE CONFLUÊNCIA ==========
        total_sinais_buy = len(sinais_buy)
        total_sinais_sell = len(sinais_sell)

        # Determina ação baseada na confluência - MÁXIMO APRENDIZADO (1+ sinal)
        if total_sinais_buy >= 1 and total_sinais_buy > total_sinais_sell:
            acao_confluencia = "BUY"
            confianca_confluencia = min(score_confluencia / 100.0, 1.0)
        elif total_sinais_sell >= 1 and total_sinais_sell > total_sinais_buy:
            acao_confluencia = "SELL"
            confianca_confluencia = min(score_confluencia / 100.0, 1.0)
        else:
            # FALLBACK: Se não há sinais claros, segue a IA
            if probabilidade_ia > 0.5:
                acao_confluencia = "BUY"
                confianca_confluencia = 0.3
            else:
                acao_confluencia = "SELL"
                confianca_confluencia = 0.3
...
class SaidaInteligentePositions:
    ...
    def verificar_saida_inteligente(self, ticket: int, preco_atual: float, rsi_atual: float) -> bool:
        ...
        if posicao["tempo_sem_lucro"] >= 300:
            logging.info(
                f"🚪 Saída por tempo sem lucro: {posicao['tempo_sem_lucro']:.0f}s")
            return True

        # CRITÉRIORSI inverteu com lucro mínimo (5 pontos WIN)
...
def forcar_recreacao_scaler():
    ...
    logging.info("🔧 Scaler global recriado com 18 features usando dados dummy")
...
class CooldownInteligente:
    ...
    def registrar_resultado(self, lucro: float):
        ...
        if lucro < -25.0:  # Loss significativo
            ...
            # Cooldown ULTRA AGRESSIVO para reduzir overtrading
            if self.losses_seguidos == 1:
                cooldown_segundos = 300  # 5 minutos após 1 loss
            elif self.losses_seguidos == 2:
                cooldown_segundos = 600  # 10 minutos após 2 losses
            else:
                cooldown_segundos = 900  # 15 minutos após 3+ losses

            self.cooldown_ativo = True
            self.fim_cooldown = time.time() + cooldown_segundos

            logging.info(
                f"⏳ Cooldown ativado: {cooldown_segundos}s após {self.losses_seguidos} loss(es)")
...
class MonitorPerformance:
    ...
    """
        ...
        """
            ...
            tempo_em_posicao = time.time() - self.tempo_inicio
            lucro_em_pontos = 0.0
            TICK_SIZE = 0.2  # Mover para config
            ...
            # REGRA 4: Trailing Stop - CORREÇÃO CRÍTICA APLICADA
            if lucro_em_pontos >= self.config['trailing_gatilho_pts']:
            novo_sl = 0.0
            distancia_trailing_preco = self.config['trailing_distancia_pts'] * TICK_SIZE
    ...
    idx_maior_bid = df_bids['volume'].idxmax()
                ...
                features['preco_maior_escora_bid'] = float(
                   maior_escora_bid.get('p', 0.0))
                    ...
                    logging.warning(
                        f"⚠️ Erro ao analisar profundidade do book: {e}")
    ...
    """
        ...
        # Durante treino ou primeira vez, cria/ajusta o scaler
        scaler_global= MinMaxScaler()
        df[colunas_numericas]= scaler_global.fit_transform(
           df[colunas_numericas])
            ...
            # Durante predição, usa o scaler já ajustado
            df[colunas_numericas] = scaler_global.transform(df[colunas_numericas])
                   ...
                   f"📦 Rotação do histórico realizada. Backup: {backup_name}")
                    ...
                    df= df.tail(5000)
                       ...
                       "✂️ Histórico truncado para últimas 5000 linhas")
                        ...
                        colunas_padrao = ['timestamp', 'action', 'bid_qty', 'ask_qty', 'spread', 'volatility',
                                 'candle_type', 'entropia_book', 'rsi_14', 'volume_tick', 'is_in_trade',
                                  'floating_profit', 'tempo_em_trade', 'lucro', 'score_distancia']
               ...
               HISTORICO_CSV) / (1024 * 1024)  # MB
                ...
                logging.warning(
                   "⚠️ Arquivo de histórico muito grande, aguardando rotação...")
                    ...
                    # Incrementa contador apenas paracias reais (operações fechadas)
                    global contador_experiencias_novas
                    if acao in ["BUY", "SELL"] and lucro != 0.0:
    ...
    """
        ...
        """Função core ROBUSTA para leitura do JSON - protege contra race conditions."""
        ...
        if os.path.getsize(BOOK_FILE_PATH) < 10:
        raise OSError(
           f"Arquivo muito pequeno: {os.path.getsize(BOOK_FILE_PATH)} bytes")

            # LEITURA ROBUSTA - Protege contra race conditions
            for encoding in ['utf-16-le', 'utf-8', 'utf-16', 'latin1']:
            try:
            with open(BOOK_FILE_PATH, 'r', encoding=encoding) as f:
            content = f.read().strip()

            # VALIDAÇÃO RIGOROSA CONTRA RACE CONDITION

            # 1. Valida tamanho mínimo (JSON real do EA tem ~500+ chars)
            if len(content) < 200:
            logging.debug(
                   f"[ler_book_csv_core] ⚠️ JSON muito pequeno ({len(content)} chars), ignorando (race condition)")
                    continue

                    # 2. Valida estrutura básica
                    if not (content.startswith('{') and content.endswith('}')):
                    logging.debug(
                    f"[ler_book_csv_core] ⚠️ JSON incompleto detectado ({encoding})")
                    continue

                    try:
                    # 3. Corrige timestamp sem aspas do EA antes do parse
                    content = _fix_timestamp(content)

                    # 4. Tenta parse do JSON
                    data = json.loads(content)

                    # 5. Valida campos obrigatórios
                    if not isinstance(data, dict):
                    logging.debug(
                       f"[ler_book_csv_core] ⚠️ JSON não é dict ({encoding})")
                        continue

                        if "bids" not in data or "asks" not in data:
                        logging.debug(
                        f"[ler_book_csv_core] ⚠️ JSON sem bids/asks ({encoding})")
                        continue

                        if not isinstance(data["bids"], list) or not isinstance(data["asks"], list):
                        logging.debug(
                        f"[ler_book_csv_core] ⚠️ bids/asks não são listas ({encoding})")
                        continue

                        # 6. Verifica se tem dados válidos (não vazios)
                        if len(data["bids"]) == 0 or len(data["asks"]) == 0:
                    logging.debug(
                        f"[ler_book_csv_core] ⚠️ bids/asks vazios ({encoding})")
                        continue

                        # 7. Validação extra: verifica se tem metadata (JSON completo do EA)
                        if "metadata" not in data:
                        logging.debug(
                        f"[ler_book_csv_core] ⚠️ JSON sem metadata, pode estar incompleto ({encoding})")
                        continue

                        # JSON VÁLIDO E COMPLETO!
                        logging.debug(
                    f"[ler_book_csv_core] ✅ JSON válido e completo ({encoding}): {len(data['bids'])} bids, {len(data['asks'])} asks")
                    return data

                    except json.JSONDecodeError as e:
                    # RACE CONDITION DETECTADA: JSON incompleto/corrompido
                    logging.debug(
                    f"[ler_book_csv_core] ⚠️ Race condition detectada (JSONDecodeError): {str(e)[:50]}...")
                    continue
            ...
            logging.debug(
                f"[ler_book_csv_core] Erro inesperado ({encoding}): {e}")
                continue

                # Se chegou aqui, todas as tentativas falharam (race condition detectada)
                logging.debug(
        "📄 [ler_book_csv_core] 🔄 Race condition confirmada - REJEITANDO leitura")

        # NÃO FAZ FALLBACK! Race condition deve ser rejeitada completamente
        raise OSError("Race condition detectada: EA ainda escrevendo arquivo")


        def ler_book_csv_with_retry() -> Optional[Dict[str, List[Dict[str, float]]]]:
        """Lê os dados do book do arquivo CSV/JSON com retry automático e validação robusta."""
        try:
...
def inicializar_mt5() -> bool:
    global trailing_stop, balanceador, detector_modo, circuit_breaker, saida_inteligente, sistema_confluencia, anti_overtrading
        ...
        # 🔧 NOVA CORREÇÃO: SISTEMA ANTI-OVERTRADING
        anti_overtrading = AntiOvertradingFilter()
        logging.info("🛡️ Sistema Anti-Overtrading inicializado (CORREÇÃO CRÍTICA)")
        ...
        # ========== INICIALIZAÇÃO NOVA MELHORIA: SISTEMA DE CONFLUÊNCIA ==========
        sistema_confluencia = SistemaConfluencia()
        logging.info(
        "🎯 Sistema de Confluência inicializado (+4% eficácia)")
        ...
        logging.info(
        "⏳ Sistema de Cooldown Inteligente inicializado (+1.5% eficácia)")
        ...
        logging.info(
        "📈 Monitor de Performance em Tempo Real inicializado (+2% eficácia)")

        logging.info("📊 Sistema de Volume ULTRA RIGOROSO ativado:")
        logging.info("   • <1000cc: 2 contratos (liquidez baixa)")
        logging.info("   • 1000-2000cc: 5 contratos (liquidez média)")
        logging.info("   • 2000-3000cc: 6 contratos (liquidez média-alta)")
        logging.info("   • 3000-5000cc: 8 contratos (liquidez top)")
        logging.info("   • 5000cc+: 10 contratos (VOLUME MONUMENTAL)")
        logging.info("   • 10cc APENAS para big players MASSIVOS!")
    ...
    logging.error(
        ...
        logging.warning(
           "Dados de mercado indisponíveis após tentativas de correção")
            ...
            # Define SL/TP baseado no VOLUME (SEGUIR BIG PLAYERS)
            volume_total = contexto_atual.get(
            'bid_qty', 0) + contexto_atual.get('ask_qty', 0)

            if volume_total >= 5000:  # VOLUME MONUMENTAL - 10cc
            sl_points_dinamico= 25   # SL agressivo (mais confiança)
                tp_points_dinamico = 50   # TP maior (aproveita movimento)
                logging.info(
               f"🚀 VOLUME MONUMENTAL (5000cc+): SL={sl_points_dinamico}, TP={tp_points_dinamico} (R/R 1:2.0)")
                elif volume_total >= 3000:  # LIQUIDEZ TOP - 8cc
                sl_points_dinamico = 30   # SL otimizado
                tp_points_dinamico = 45   # TP 1:1.5 ratio
                logging.info(
                f"🏆 LIQUIDEZ TOP (3000cc+): SL={sl_points_dinamico}, TP={tp_points_dinamico} (R/R 1:1.5)")
                elif volume_total >= 2000:  # LIQUIDEZ MÉDIA-ALTA - 6cc
                sl_points_dinamico = 30   # SL otimizado
                tp_points_dinamico = 45   # TP 1:1.5 ratio
                logging.info(
                f"⭐ LIQUIDEZ MÉDIA-ALTA (2000cc+): SL={sl_points_dinamico}, TP={tp_points_dinamico} (R/R 1:1.5)")
                else:  # LIQUIDEZ BAIXA/MÉDIA - 2-5cc
                sl_points_dinamico = 35   # SL mais conservador
                tp_points_dinamico = 40   # TP menor (menos ambição)
                logging.info(
                f"✅ LIQUIDEZ BAIXA/MÉDIA: SL={sl_points_dinamico}, TP={tp_points_dinamico} (R/R 1:1.14)")
                else:
    ...
    logging.error(
        ...
        # 🔧 NOVA CORREÇÃO: Registra operação no sistema anti-overtrading
        if anti_overtrading:
            anti_overtrading.register_trade(action)
            logging.debug(f"🛡️ Operação {action} registrada no anti-overtrading")
       ...
       f"🎯 Score distância calculado para ticket {ticket_ordem_abertura}: {score_dist:.4f}")
        ...
        logging.warning(
           "⚠️ Informações do símbolo indisponíveis para trailing")
            ...
            if freeze_level == 0:
            freeze_level = 10  # Padrão para WIN: 10 pontos mínimo

            # Adicionar margem de segurança de 50% sobre o freeze level
            ...
            distancia_atual = abs(novo_sl - preco_referencia)
...
@ app.route("/lucro")
    ...
    # 🔧 CORREÇÃO CRÍTICA: Treina a cada 5 experiências novas (era 10)
    LIMITE_EXPERIENCIAS_PARA_TREINO = 5
    ...
    try:
        ...
        if not verificar_e_proteger_modelo():
            logging.warning(
                ...
                logging.warning(
                   "⚠️ Modelo não encontrado, criando novo modelo...")
                    ...
                    # 1. Gerenciador de Saída Unificado
                    config_saida = {
            'timeout_sem_evolucao_s': 120,       # 2 minutos
            'lucro_minimo_evolucao_pts': 5,      # 5 pontos
            'pico_minimo_protecao_pts': 20,      # Começa a proteger após 20 pts de lucro
            'percentual_perda_pico': 0.3,        # Sai se perder 30% do lucro máximo
            'tempo_max_estagnacao_s': 180,       # 3 minutos
            'lucro_max_estagnacao_pts': 15,      # Lucro considerado "pequeno"
            'trailing_gatilho_pts': 20,          # Ativa trailing com 20 pontos
            'trailing_distancia_pts': 10         # Mantém 10 pontos de distância
        }
            ...
            # CONTADOR DE REJEIÇÕES PARA MODO EMERGÊNCIA
            ...
            posicao_atual = None
                   ...
                   (p for p in current_positions if p.magic == MAGIC_NUMBER), None) if current_positions else None
                    ...
                    logging.info(
                           ...
                           posicao_ativa_no_mt5)
                            ...
                            # Verificação de segurança antes de fechar
                            if posicao_atual is not None:
                            fechar_posicao_atual(motivo)
                            else:
                            logging.warning(
                                   f"⚠️ posicao_atual é None. Usando fechar_todas_posicoes() como fallback.")
                                    fechar_todas_posicoes(motivo)
                   ...
                   'bid_qty', 0) + contexto.get('ask_qty', 0)
                    ...
                    if contador_rejeicora_consecutivas >= LIMITE_REJEICOES_EMERGENCIA:
                    logging.warning(
                        ...
                        acao_predita, confianca_predita= prever_acao(
                           modelo_ia_local, X_decisao, modo_operacional,
                            ...
                            prob_bruta= modelo_ia_local.predict(
                                ...
                                # Sobrescrever decisão da IA se confluência discordar
                                if confluencia_info['acao'] != "NADA":
                                if confluencia_info['acao'] != acao_predita:
                                logging.warning(
                                       f"🎯 CONFLUÊNCIA SOBRESCREVE: IA={acao_predita} → CONFLUÊNCIA={confluencia_info['acao']}")
                                        acao_para_executar = confluencia_info['acao']
                                        confianca_decisao = confluencia_info['confianca']
                                        else:
                                        # Confluência confirma IA - aumenta confiança
                                        acao_para_executar = acao_predita
                                        confianca_decisao = min(
                                        confianca_predita * 1.2, 1.0)
                                        logging.info(
                                        f"🎯 CONFLUÊNCIA CONFIRMA: {acao_predita} | Confiança aumentada: {confianca_decisao:.2f}")
                                        else:
                                        # Confluência não encontrou sinais suficientes
                                        acao_para_executar = "NADA"
                                        confianca_decisao = 0.0
                                        logging.info(
                                    f"🎯 CONFLUÊNCIA BLOQUEIA: Sinais insuficientes para {acao_predita}")
                                    else:
                                    acao_para_executar = acao_predita
                    ...
                    memoria_experiencias.adicionar(
                       contexto.copy(), "NAO_AGIU", 0.0, 0.0)
                        ...
                        # 🔧 NOVA CORREÇÃO: FILTRO ANTI-OVERTRADING
                        if acao_para_executar in ["BUY", "SELL"] and anti_overtrading:
                            pode_operar, motivo = anti_overtrading.can_trade(acao_para_executar)
                            if not pode_operar:
                                logging.warning(f"🛡️ ANTI-OVERTRADING: Operação {acao_para_executar} bloqueada - {motivo}")
                                acao_para_executar = "NAO_AGIU_OVERTRADING"
                                # Salva experiência de bloqueio por overtrading
                                memoria_experiencias.adicionar(
                                    contexto.copy(), "NAO_AGIU", 0.0, 0.0)
                                salvar_experiencia_csv(
                                    contexto.copy(), "NAO_AGIU", 0.0, 0.0)
    ...
    def __init__(self, max_size: int = MAX_EXPERIENCIAS_MEMORIA):
        ...
        return batch, decays
            ...
            logging.info(
                ...
                logging.info(
                f"📚 Carregamento balanceado: {len(reais_recentes)} operações reai{len(nao_agiu_recentes)} NAO_AGIU")
                ...
                logging.info(
                   f"📊 Razão BUY/SELL final: {self.razao_buy_sell:.3f} ({self.contagem_acoes.get('BUY', 0)}/{total_ops})")
                    ...
                    logging.error(f"❌ Erro ao carregar experiências do CSV: {e}")
    ...
    """
        ...
        logging.info(
            ...
            **ctx, "action": ac, "reward": luc, "score_dist": score_dist
            ...
            # 4. AVALIAR O MODELO ATUAL (ANTES DO TREINO) no conjunto de validação
            ...
            # Força build do modelo e recria otimizador
            ...
            verbose = 0,
            # INCLUIR DADOS DE VALIDAÇÃO NO FIT
            ...
            # 6. COMPARAR O MODELO NOVO COM O ANTIGO
           ...
           f"Performance do Modelo NOVO na validação: Loss={loss_nova:.4f}, Acurácia={acc_nova:.4f}")
            ...
            # 7. DECIDIR SE SALVA O MODELO
            if loss_nova < loss_antiga:
            logging.info(
               f"✅ MELHORIA DETECTADA! Loss caiu de {loss_antiga:.4f} para {loss_nova:.4f}. Salvando novo modelo.")
                salvar_modelo(modelo)  # Salva o modelo melhorado
                # Salva o histórico de loss da validação
                historico_loss.extend(history.history['val_loss'])
                else:
                logging.warning(
                f"❌ SEM MELHORIA. Loss aumentou ou estagnou ({loss_antiga:.4f} -> {loss_nova:.4f}). Modelo antigo mantido.")
                # Como não salvamos, o modelo em memória (que foi treinado) será descartado na próxima inicialização.
                # Para garantir, poderíamos recarregar o modelo antigo do disco.
                # Recarrega a versão boa do disco
    ...
    """
        ...
        # SDO PARA APRENDIZADO (mínimo 6 de 11 possíveis) - SELETIVIDADE BALANCEADA
        if score_qualidade < 6:
        CONTADOR_OPERACOES_REJEITADAS += 1

        # APRENDIZADO FORÇADO: Após muitas rejeições, opera mesmo com score baixo
        if CONTADOR_OPERACOES_REJEITADAS >= LIMITE_REJEICOES_PARA_APRENDIZADO:
        CONTADOR_OPERACOES_REJEITADAS = 0
            MODO_APRENDIZADO_FORCADO= True
            logging.warning(
               f"🎓 APRENDIZADO FORÇADO ATIVADO! Score baixo aceito: {score_qualidade}/11 para gerar experiências")
                return True, f"Aprendizado forçado (score {score_qualidade}/11) - Gerando experiências"

                return False, f"Setup de baixa qualidade: score {score_qualidade}/11 < 6 (Rejeições: {CONTADOR_OPERACOES_REJEITADAS}/{LIMITE_REJEICOES_PAIZADO})"

                # Reset contador qu é bom
                CONTADOR_OPERACOES_REJEITADAS = 0

                logging.info(
        f"✅ SETUP APROVADO PARA APRENDIZADO! Score: {score_qualidade}/11 | Vol: {volume_total} | Entropia: {entropia:.3f} | ATR: {atr:.1f} | RSI: {rsi:.1f}")
        return True, f"Setup aprovado para aprendizado (score {score_qualidade}/11)"
        ...
        def prever_acao(modelo: Sequential, X: pd.DataFrame,
               modo_operacional: Optional[ModoOperacional] = None,
                ...
                """Prevê a próxima ação com filtros de MÁXIMA ACERTIVIDADE."""
                try:
            ...
            logging.warning(
               f"⚠️ Probabilidade muito baixa: {acao_prob:.6f} - Modelo pode precisar de retreino")
                ...
                logging.info(f"⚖️ Balanceamento: BUY={status['buy_count']}, SELL={status['sell_count']}, "
                    ...
                    logging.info(
                    f"⏳ Operação bloqueada - Cooldown ativo ({tempo_restante}s restantes)")
                    return "NADA", 0.0
           ...
           f"[salvar_decisao_csv] Arquivo '{abs_path_arquivo}' existe: {file_exists}, Tamanho: {file_size} bytes")
            ...
            logging.info(
               "📅 Fim de semana: book não disponível (comportamento normal)")
                ...
                if agora < inicio_pregao or agora > fim_pregao:
                logging.info(
                f"🕐 Mercado fechado ({agora.strftime('%H:%M')}): usando dados do arquivo CSV")
                ...
                logging.warning("⚠️ Arquivo CSV vazio ou inválido")
                    return False
                except Exception as e:
    ...
    fechar_posicao_score(
        ...
        logging.error(
               "❌ Erro crítico: mt5.order_send retornou None (falha de conexão)")
                ...
                if resultado.retcode == mt5.TRADE_RETCODE_DONE:
                logging.info(f"✅ Posição {posicao_atual.ticket} fechada: {motivo}")
                return True
                else:
    ...
    monstro_thread_obj.join()
                                             ...
                                             # Defina a data de expiração (Ano, Mês, Dia)
                                             EXPIRATION_DATE = datetime(2025, 10, 26)

                                             if datetime.now() > EXPIRATION_DATE:
    print("Prazo expirado. Solicite nova versão.")
    import sys
    sys.exit()
                                             # ======================================
