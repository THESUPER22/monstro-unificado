...
def checar_arquivos_essenciais() -> Tuple[bool, List[str]]:
    ...
    """
    # Arquivos REALMENTE usados pelo robô WIN v2 (nomes corrigidos — antes apontavam
    # para nomes antigos/WDO que nunca existem e geravam aviso a cada diagnóstico).
    arquivos_essenciais = [
    ...
    # Verifica arquivos opcionais — se faltar, é normal (será criado). Fica em DEBUG
    # para NÃO poluir o log a cada diagnóstico periódico.
    for arquivo in arquivos_opcionais:
        if not os.path.exists(arquivo):
            logging.debug(
                f"Arquivo opcional ainda não existe: {arquivo} (será criado automaticamente)")
    ...
    if sucesso:
        logging.debug("✅ Diagnóstico de arquivos: OK")
    else:
