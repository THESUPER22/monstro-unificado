...
def _get_rompimento_status():
    """Retorna status do modulo Rompimento da 1a Hora para o dashboard."""
    try:
        import json
        import os
        ...
        with open(cfg_path, 'r', encoding='utf-8') as f:
            cfg = json.load(f)
        rp_cfg = cfg.get('rompimento', {})

        state_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'logs', 'rompimento', 'rompimento_state.json')
        state = {}
        ...
        ativo = bool(rp_cfg.get('ativo', False))
        fim = float(rp_cfg.get('hora_fim', 11.0))
        na_janela = ativo and 9.0 <= h < fim

        if now.weekday() >= 5:
            proxima = 'FORA (fim de semana)'
        elif na_janela:
            proxima = 'GATILHO ATIVO'
        elif h < 9.0:
            proxima = '09:00 (caixa)'
        else:
            proxima = 'ENCERRADA'
        ...
        try:
            import csv
            csv_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'logs', 'rompimento', 'rompimento_trades.csv')
            if os.path.exists(csv_path):
                with open(csv_path, encoding='utf-8') as f:
                    reader = csv.DictReader(f)
                    for row in reader:
                        if row.get('dia') == now.date().isoformat() and row.get('pts'):
                            pts = float(row['pts'])
                            pnl_hoje += pts * 10 * 5  # pts * R/pt * 5 CC
                            ops_hoje += 1
        except Exception:
            pass
        ...
        return {
            'ativo': ativo,
            'estado_faixa': 'ROMPIMENTO_EXCLUSIVO' if na_janela else 'INATIVO',
            'volume': rp_cfg.get('lote', 5.0),
            'magic': rp_cfg.get('magic', 7008),
            'sl_tp': "SL " + str(rp_cfg.get('sl_mode', 'lo')) + " / TP " + str(rp_cfg.get('tp_k', 1.5)) + "x",
...
def api_status():
    ...
    try:
        ...
        return jsonify({
            ...
            'trades': trades_list,
            # ===== 7 VELAS STATUS =====
            'rompimento': _get_rompimento_status()
        })
    except Exception as e:
