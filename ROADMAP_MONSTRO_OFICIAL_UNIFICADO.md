...
- **FIX 11/09/2026 (noite, parte 2) — watchdog escalado no v22:** `executar_resgate_dll` mantém o resgate "soft" (shutdown+initialize) nas primeiras ocorrências, mas após **≥5 resgates consecutivos sem o feed voltar** (≈1–2 min congelado) **escala para o restart do PROCESSO `terminal64.exe`** (`_restart_terminal_processo`: taskkill + 6s + launch + 18s + re-connect) — o único caminho que destrava o coletor congelado. Guarda-roupas: no máx. 1 restart a cada 5 min (`_WATCHDOG_ESCALADA_COOLDOWN_S`) e teto diário de 4 (`_WATCHDOG_ESCALADA_MAX_DIA`); após o teto volta ao soft. Posição aberta é segura (SL/TP no servidor). Não altera gatilho/entrada/saída (Protocolo 04/09). Testes novos PA6–PA8 em `tests/teste_watchdog_e_fecho.py` (escalada/cooldown/teto/reset da sequência).
- **FIX 11/09/2026 (noite, parte 3) — agente vira 2ª camada de feed:** `agente_monstro_core.py` ganhou probe de tick direto no terminal (`feed_spot_stale_s`, usando o mesmo front dinâmico do robô — jamais `WDO$`, que está congelado): com o robô vivo e em expediente, se o tick do front estagna além de `feed_estagnado_max_s` (=900 s) o agente reinicia o **processo** do terminal (respeitando o backoff compartilhado); se o terminal caiu completamente com robô vivo, só reabre o terminal. Config novo: `rotinas.feed_watchdog_enabled` e `rotinas.feed_estagnado_max_s`.
