...
- **FIX 11/09/2026 — reset do MT5 no start:** `start_all.bat` e `iniciar_v22_wdo.bat` agora **matam e reabrem o `terminal64.exe` antes de subir o robô** (taskkill + 5s + start + 15s). Motivo: terminal que "dormiu aberto" + ponte Python anexada = coletor de dados congelado (11/09: 2.553× "Tick estagnado", Faixa 1 S/TRADE com feed ~180 min velho, mesmo com `WDOV26` tendo o pregão inteiro). Instância nova lê dados normalmente. Observação separada: o símbolo contínuo `WDO$` do XPMT5-DEMO congelou em 10/09 — o robô usa o front dinâmico (`get_front_month_symbol_dynamic` → `WDOV26`), não `WDO$`; o `WDO$` serve só para backtests/paridade.
- **FIX 11/09/2026 (noite, parte 2) — watchdog escalado no v22:** `executar_resgate_dll` mantém o resgate "soft" (shutdown+initialize) nas primeiras ocorrências, mas após **≥5 resgates consecutivos sem o feed voltar** (≈1–2 min congelado) **escala para o restart do PROCESSO `terminal64.exe`** (`_restart_terminal_processo`: taskkill + 6s + launch + 18s + re-connect). Guarda-roupas: no máx. 1 restart a cada 5 min (`_WATCHDOG_ESCALADA_COOLDOWN_S`) e teto diário de 4 (`_WATCHDOG_ESCALADA_MAX_DIA`); após o teto volta ao soft. Posição aberta é segura (SL/TP no servidor). A escalada mede a **idade REAL dos dados** (`_stale_real_min`: última M5 do `SYMBOL`, fallback tick — epoch absoluto, fuso-independente), não o heartbeat do loop principal (que o próprio loop re-arma a cada iteração e mascarava os episódios — em 14/09 `FEED CONGELADO` nunca disparou por isso). Não altera gatilho/entrada/saída (Protocolo 04/09). Testes PA6–PA8 em `tests/teste_watchdog_e_fecho.py`.
...
- **VEREDITO 14/09/2026 — feed do XPMT5-DEMO entregando o pregão com ~3h de atraso:** mesmo com o terminal 100% novo no start (`start_all`) e **33 restarts do processo** feitos pelo agente das 09:05 às 17:20, a última M5/último tick NUNCA saiu de ~3h de idade (10800 s constantes em todos os probes; `defasado 180–185 min` no orquestrador). À noite o histórico de HOJE aparece completo (114 M5 até 15:25, 570 M1 06:00–15:29, WDOV26 com bid real 5163.0 — mesmo cutoff normal do demo). Conclusão: **o mesmo assinatura de 11/09 (defasado 180–185 min) se repete com terminal limpo → a causa é o SERVIDOR de demo (stream/delivery do pregão defasado em ~3h), não o terminal/coletor**. Nenhum restart client-side conserta o problema nesse cenário. Dia 14/09: 0 decisões, 0 trades, Faixa 1 S/TRADE. Pendência para a corretora: confirmar se o feed real (conta real/não-demo) entrega o pregão em tempo real e cobre a tarde (as barras M5 do demo já cortam às ~15:25).
