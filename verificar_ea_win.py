#!/usr/bin/env python3
# -*- coding: utf-8 -*-
RIFICADOR DO EA WIN - Diagnóstico do Expert Advisor
Verifica se o EA está funcionando corretamente
"""
...
def verificar_ea_ativo():
    """Verifica se há EAs ativos"""
    print("\n🤖 VERIFICANDO EAs ATIVOS:")
    print("=" * 30)

    # Não há função direta para listar EAs, mas podemos verificar ordens
    orders = mt5.orders_get()
    positions = mt5.positions_get()

    print(f"   📋 Ordens ativas: {len(orders) if orders else 0}")
    print(f"   📊 Posições abertas: {len(positions) if positions else 0}")

    # Verifica se há magic numbers específicos
    if positions:
        magic_numbers = set()
        for pos in positions:
            magic_numbers.add(pos.magic)

        print(f"   🎯 Magic numbers encontrados: {magic_numbers}")

        # Magic 123457 é do nosso EA
        if 123457 in magic_numbers:
            print(f"   ✅ EA Monstro detectado (Magic: 123457)")
        else:
            print(f"   ⚠️  EA Monstro não detectado")
